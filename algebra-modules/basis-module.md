...
x=\sum_{b\in B} r_b\, b
\]
with coefficients $r_b\in R$, where all but finitely many $r_b$ are zero. Uniqueness is equivalent to the familiar notions of {{< knowl id="linearly-independent-and-linearly-dependent-sets" section="convex-analysis" text="linear independence" >}} and spanning (compare {{< knowl id="linear-combination" section="convex-analysis" text="linear combinations" >}} in linear algebra).
