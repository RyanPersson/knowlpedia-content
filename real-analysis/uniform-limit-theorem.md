...
The **uniform limit theorem** states: if \((f_n)\) is a sequence of {{< knowl id="continuous-function" text="continuous functions" >}} \(f_n: X \to \mathbb{R}\) on a {{< knowl id="metric-space" section="topology" text="metric space" >}} \(X\), and \(f_n \to f\) {{< knowl id="uniform-convergence" text="uniformly" >}}, then \(f\) is continuous.
