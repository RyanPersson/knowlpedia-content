...
Let $U\subseteq\mathbb{R}^k$ be {{< knowl id="open-set" section="topology" text="open" >}} and let $f:U\to\mathbb{R}^m$. The function $f$ is **(Fréchet) differentiable at $a\in U$** if there exists a {{< knowl id="linear-map" section="linear-algebra" text="linear map" >}} $A:\mathbb{R}^k\to\mathbb{R}^m$ such that
$$
