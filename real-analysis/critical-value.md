...
Equivalently, $y$ is critical if the fiber $F^{-1}(\{y\})$ contains at least one point where the {{< knowl id="jacobian-matrix" text="Jacobian matrix" >}} fails to have full rank. Values that are not critical are precisely the {{< knowl id="regular-value" text="regular values" >}}.
