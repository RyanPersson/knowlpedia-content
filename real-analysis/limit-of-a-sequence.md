...
Let $(X,d)$ be a {{< knowl id="metric-space" section="topology" text="metric space" >}} and let $(x_n)$ be a sequence in $X$. A point $x\in X$ is the **limit** of $(x_n)$ if
...
The limit (when it exists) summarizes the eventual behavior of a sequence. In metric spaces, a sequence has at most one limit (see {{< knowl id="convergent-sequence" section="topology" text="convergent sequence" >}}).
