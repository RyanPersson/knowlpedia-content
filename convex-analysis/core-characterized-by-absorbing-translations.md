...
Let $X$ be a {{< knowl id="vector-space" section="linear-algebra" text="vector space" >}} and $\Omega\subset X$ be nonempty. For $x\in X$, consider the translate $\Omega-x:=\{w-x\mid w\in\Omega\}$ (using {{< knowl id="set-operations-sum-scalar-multiple-difference" text="set difference/translation notation" >}}).
