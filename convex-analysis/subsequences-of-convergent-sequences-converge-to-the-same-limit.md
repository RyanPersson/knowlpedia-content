...
Let $(X,d)$ be a {{< knowl id="metric-metric-space" text="metric space" >}}, and let $(x_n)$ be a sequence in $X$ that {{< knowl id="convergence-of-a-sequence" text="converges" >}} to $a\in X$. If $(x_{n_k})$ is a {{< knowl id="subsequence" section="analysis" section="analysis" text="subsequence" >}} of $(x_n)$, then $x_{n_k}\to a$.
