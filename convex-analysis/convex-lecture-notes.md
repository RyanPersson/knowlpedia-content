...
**Definition 1.55 (Cauchy sequence).** Let $(X,d)$ be a metric space. A sequence $(x_n)$ in $X$ is called a {{< knowl id="cauchy-sequence" section="topology" text="Cauchy sequence" >}} if for each $\varepsilon>0$, there exists $N\in\mathbb{N}$ such that $d(x_m,x_n)<\varepsilon$ for all $m,n\ge N$.
...
Then $(x_{n_k})_{k}$ is a sequence in $X$ and it is called a {{< knowl id="subsequence" section="real-analysis" section="analysis" text="subsequence" >}} of $(x_n)$.
...
**Proposition 1.68.** ({{< knowl id="reverse-triangle-inequality" section="real-analysis" text="Reverse triangle inequality" >}}) Let $(X,\|\cdot\|)$ be a normed space. Then
$$
