...
**Hahn–Banach (real version)**: Let $X$ be a real {{< knowl id="vector-space" section="linear-algebra" section="linear-algebra" text="vector space" >}}, let $Y\subset X$ be a {{< knowl id="linear-subspace" text="linear subspace" >}}, and let $p:X\to\mathbb{R}$ be {{< knowl id="subadditive-positively-homogeneous-and-sublinear-functions" text="sublinear" >}}.
