...
Let $X$ be a {{< knowl id="vector-space" section="linear-algebra" section="linear-algebra" text="vector space" >}} over $\mathbb{R}$ (or $\mathbb{C}$). A **norm** on $X$ is a function $\|\cdot\|:X\to[0,\infty)$ such that for all $x,y\in X$ and scalars $\alpha$:
