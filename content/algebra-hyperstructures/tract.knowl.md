...
A **tract** is a pair \(F=(G,N_F)\), where \(G\) is an abelian group and \(N_F\subseteq\mathbb N[G]\) is a set of formal sums, called the **null set**, satisfying:
...
The underlying pointed multiplicative set is \(F=G\sqcup\{0\}\), with \(F^\times=G\). A morphism of tracts is a group homomorphism \(G\to G'\) whose linear extension \(\mathbb N[G]\to\mathbb N[G']\) carries \(N_F\) into \(N_{F'}\).
