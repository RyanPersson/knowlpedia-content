...
A connected closed manifold has an integral fundamental class exactly when it is orientable. Every closed manifold has a canonical mod-\(2\) fundamental class. If \(M\) has boundary, the oriented class lies instead in \(H_n(M,\partial M;R)\); if \(M\) is noncompact, the corresponding object belongs to locally finite homology. These variants preserve the same local-generator condition [Hatcher, §3.3].
