...
prerequisites = ["topology/closed-manifold", "topology/singular-homology-group", "topology/relative-singular-homology", "algebra-rings/commutative-ring"]
...
\]
is the local orientation generator at every \(x\in M\). Thus \([M]\) is the global homology class that consistently assembles all local choices of orientation.

## Smooth manifolds

For a [[fiber-bundles/smooth-manifold|smooth manifold]], an [[fiber-bundles/orientation-of-a-real-vector-bundle|orientation of the tangent bundle]] determines the corresponding integral fundamental class.

## Existence and variants

A connected closed manifold has an integral fundamental class exactly when it is orientable. Every closed manifold has a canonical mod-\(2\) fundamental class. If \(M\) has boundary, the oriented class lies instead in [[topology/relative-singular-homology|relative homology]] \(H_n(M,\partial M;R)\); if \(M\) is noncompact, the corresponding object belongs to locally finite homology. These variants preserve the same local-generator condition.
...
The standard orientation of \(S^n\) for \(n>0\) selects one of the two generators of \(H_n(S^n;\mathbb Z)\cong\mathbb Z\). Reversing the orientation replaces \([M]\) by \(-[M]\). For a closed nonorientable manifold, no integral class satisfies the local-generator condition, although its mod-\(2\) fundamental class still exists.
