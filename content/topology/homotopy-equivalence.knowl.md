...
Two [[topology/topological-space|topological spaces]] \(X\) and \(Y\) are **homotopy equivalent** if there are [[topology/continuous-map|continuous maps]] \(f:X\to Y\) and \(g:Y\to X\) such that \(g\circ f\) is homotopic to \(\operatorname{id}_X\) and \(f\circ g\) is homotopic to \(\operatorname{id}_Y\). A homotopy is a continuous one-parameter deformation between maps.
