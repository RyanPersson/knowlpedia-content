...
such that either \(A=\varnothing\), or there exist \(x_0\in X\) and \(R>0\)
such that
\[
A\subseteq B_d(x_0,R),
\]
where \(B_d(x_0,R)\) is the [[topology/open-ball|open ball]] of radius \(R\)
around \(x_0\).
...
Boundedness is a basic size condition that appears in notions such as [[topology/totally-bounded-set|total boundedness]].
