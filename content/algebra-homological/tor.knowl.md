...
M \otimes_R N,
\]
see [[algebra-modules/tensor-product|tensor product]]. (If $R$ is commutative, one may treat both as left $R$-modules.)

## Definition (via a projective resolution)
Choose a [[algebra-homological/projective-resolution|projective resolution]] $P_\bullet \to M$ by projective right $R$-modules:
\[
\cdots \to P_2 \to P_1 \to P_0 \to M \to 0.
\]
Tensor with $N$ to obtain a [[algebra-homological/chain-complex|chain complex]] $P_\bullet \otimes_R N$, and define
\[
\mathrm{Tor}^R_n(M,N)\;:=\; H_n(P_\bullet \otimes_R N),
\]
where $H_n(-)$ denotes [[algebra-homological/homology-module|homology]].
