...
kind = "knowl"
summary = "In a commutative 3×3 diagram in an abelian category with exact rows/columns, exactness of one row (or column) follows from the other eight exact sequences."
...
\end{array}
\]
whose columns and first two rows are [[algebra-category-theory/exact-sequence-categorical|exact]] (equivalently, short exact in each line).
