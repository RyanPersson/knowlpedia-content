+++
id = "algebra-homological/hom-tensor-exactness"
...
### Hom is left exact
For any fixed \(M\), the functor \(\operatorname{Hom}_R(M,-)\) is [[algebra-homological/hom-left-exact|left exact]]:
...
**Projectivity criterion.** \(\operatorname{Hom}_R(M,-)\) is exact (i.e. also right exact) iff \(M\) is [[algebra-modules/projective-module|projective]].

### Tensor is right exact
For any fixed \(N\), the functor \(-\otimes_R N\) is [[algebra-homological/tensor-right-exact|right exact]]:
...
**Flatness criterion.** \(-\otimes_R N\) is exact (i.e. also left exact) iff \(N\) is [[algebra-modules/flat-module|flat]].
...
**Injectivity criterion.** \(\operatorname{Hom}_R(-,N)\) is exact (i.e. also right exact in this contravariant direction) iff \(N\) is [[algebra-modules/injective-module|injective]].
...
See [[algebra-homological/ext-tor-derived-functors|Ext and Tor as derived functors]] and [[algebra-homological/ext-tor-derived-functors|derived-functor definitions]].
...
\]
illustrating that \(\mathbb Z/n\) is not projective over \(\mathbb Z\) (see [[algebra-homological/ext|Ext]]).
