...
kind = "knowl"
summary = "The bosonic Fock space ⊕_{n≥0} Sym^n(H) with vacuum vector"
aliases = ["symmetric-fock-space", "Symmetric Fock Space S(H)"]
domains = ["shale-paper"]
legacy_source_path = "shale-paper/symmetric-fock-space.md"
+++

The **symmetric (bosonic) Fock space** over \(H\) is
\[
S(H)=\bigoplus_{n=0}^\infty \mathrm{Sym}^n(H),
\]
with vacuum vector \(e_0\in \mathrm{Sym}^0(H)\cong \mathbb C\).
