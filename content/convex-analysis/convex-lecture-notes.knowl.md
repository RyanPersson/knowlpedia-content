...
We say that $(X,+,\cdot)$ is a [[linear-algebra/vector-space|vector space / linear space]] over the field $K$ if the following properties are satisfied:
...
Let $X$ be a vector space over $K$ and let $x_1,x_2,\dots,x_m$ be vectors in $X$. A [[convex-analysis/linear-combination|linear combination]] of $x_1,x_2,\dots,x_m$ is a vector in $X$ of the form
$$
...
**Definition 1.6.** Let $M$ be a subset of a vector space $X$. We say that $M$ is [[convex-analysis/linearly-independent-and-linearly-dependent-sets|linearly independent]] if for any finite set $\{x_1,\dots,x_m\}\subset M$ and scalars $\alpha_1,\dots,\alpha_m\in K$ we have the implication
$$
...
**Definition 1.7.** Let $B$ be a subset of a vector space $X$. The set $B$ is called a [[convex-analysis/basis-hamel-basis-and-dimension|basis]] (or **Hamel basis**) of $X$ if the following conditions are satisfied:
...
**Proposition 1.8.** ([[convex-analysis/basis-characterized-by-maximal-linear-independence|Basis characterized by maximal linear independence]]) Let $B$ be a nonempty subset of a vector space $X$. Then $B$ is a basis of $X$ if and only if both of the following conditions are satisfied:
...
**Theorem 1.9.** ([[convex-analysis/extension-of-a-linearly-independent-set-to-a-basis|Extension of a linearly independent set to a basis]]) Let $X$ be a vector space and let $\varnothing\neq M$ be a linearly independent set in $X$. Then there exists a basis $B$ of $X$ such that $B\supset M$.
...
**Corollary 1.10.** ([[convex-analysis/existence-of-a-basis-hamel-basis|Existence of a basis]]) Every vector space $X\neq\{0\}$ has a basis.
...
**Definition 1.15.** Let $(X,+,\cdot)$ be a vector space over a field $K$ and let $Y$ be a subset of $X$. The set $Y$ is said to be a [[convex-analysis/linear-subspace|linear subspace]] of $X$ if:
...
**Proposition 1.16.** ([[convex-analysis/subspace-test-closure-under-addition-and-scalar-multiplication|Subspace test]]) Let $Y$ be a nonempty subset of a vector space $X$ over a field $K$. Then $Y$ is a linear subspace of $X$ if and only if:
...
**Proposition 1.20.** ([[convex-analysis/intersection-of-subspaces-is-a-subspace|Intersection of subspaces is a subspace]]) The intersection of a family of linear subspaces of a vector space $X$ is also a linear subspace of $X$.
...
**Definition 1.21.** Let $A$ be a subset of a vector space $X$. The intersection of all linear subspaces of $X$ containing $A$ is said to be the **linear subspace generated by $A$** or the [[convex-analysis/subspace-generated-by-a-set-span|span]] of $A$. It is denoted by $\langle A\rangle$, $\operatorname{span}A$, or $\operatorname{lin}A$.

According to the definition, $\operatorname{span}A$ is the smallest linear subspace of $X$ containing the set $A$.

**Theorem 1.22.** ([[convex-analysis/span-equals-the-set-of-all-finite-linear-combinations|Span equals the set of all finite linear combinations]]) Let $X$ be a vector space. The subspace generated by a subset $A$ of $X$ is the set of all linear combinations of elements of $A$.
...
**Proposition 1.23.** ([[convex-analysis/sum-of-subspaces-equals-span-of-the-union|Sum of subspaces equals span of the union]]) Let $M$ and $N$ be two linear subspaces of a vector space $X$. Then $M+N$ is also a subspace of $X$. Furthermore, we have the representation
$$
...
**Definition 1.24.** Let $X$ be a vector space and let $M,N$ be two linear subspaces of $X$. If $Y=M+N$ and $M\cap N=\{0\}$, then $Y$ is called the [[convex-analysis/direct-sum-of-subspaces|direct sum]] of $M$ and $N$ denoted $Y=M\oplus N$.

**Theorem 1.25.** ([[convex-analysis/characterization-of-direct-sums|Characterization of direct sums]]) Let $M$ and $N$ be two linear subspaces of a vector space $X$ and let $Y$ be a subset of $X$. Then $Y=M\oplus N$ if and only if any element $y\in Y$ has a unique representation $y=a+b$ for $a\in M$ and $b\in N$.
...
It can be shown that with these two operations, $X$ becomes a vector space over $K$ which is called the [[convex-analysis/product-space-cartesian-product|product space]] (or **direct product**) of the $m$ vector spaces $X_1,\dots,X_m$.
...
**Definition 1.27.** Let $Y$ be a linear subspace of a vector space $X$. Then $X/Y$ with the addition and scalar multiplication defined in (1.2) is a vector space, which is called a [[convex-analysis/quotient-vector-space-codimension|quotient vector space]]. The dimension of $X/Y$ is called the [[convex-analysis/codimension|codimension]] of $Y$ denoted by $\operatorname{codim}Y$ (or $\operatorname{codim}(Y)$), i.e.,
$$
...
**Definition 1.28.** Let $X$ and $Y$ be two vector spaces over a field $K$. A function $T:X\to Y$ is called a [[convex-analysis/linear-operator-linear-transformation|linear operator]] (or a **linear transformation**) if:
...
Let $X$ and $Y$ be vector spaces over a field $K$ and let $T:X\to Y$ be a linear operator. The set $\operatorname{im}T=T(X)$ is called the [[convex-analysis/image-and-kernel-linear-isomorphism|image]] of $T$, and the set $\ker T=T^{-1}(0)$ is called the [[convex-analysis/image-and-kernel-linear-isomorphism|kernel]] of $T$. If $T$ is a bijection, we say that it is a [[convex-analysis/image-and-kernel-linear-isomorphism|linear isomorphism]] and the spaces $X$ and $Y$ are said to be **isomorphic**, and we write $X\cong Y$.

**Proposition 1.29.** ([[convex-analysis/images-and-preimages-of-subspaces-under-linear-operators|Images and preimages of subspaces]]) Let $T:X\to Y$ be a linear operator between vector spaces $X$ and $Y$.
...
**Theorem 1.30.** ([[convex-analysis/isomorphism-theorem-and-dimension-formula-for-linear-operators|Isomorphism theorem]]) Let $T:X\to Y$ be a linear operator between vector spaces $X$ and $Y$. Then
$$
...
**Definition 1.31.** Let $X$ be a nonempty set. A function $d:X\times X\to\mathbb{R}$ is called a [[convex-analysis/metric-metric-space|metric]] on $X$ if the following hold for all $x,y,z\in X$:

(1) $d(x,y)\ge 0$, and $d(x,y)=0$ if and only if $x=y$.
(2) $d(x,y)=d(y,x)$.
(3) $d(x,z)\le d(x,y)+d(y,z)$ (the **triangle inequality**).

The set $X$ together with the metric $d$ is called a [[convex-analysis/metric-metric-space|metric space]] and is denoted by $(X,d)$. If the metric $d$ has been specified on $X$ so that no confusion will occur, we can say simply that $X$ is a metric space.
...
(i) The [[convex-analysis/open-and-closed-balls-in-a-metric-space|open ball]] in $X$ of center $x_0$ and radius $r$ is the set
$$
B(x_0;r):=\{x\in X\mid d(x,x_0)<r\}.
$$

(ii) The [[convex-analysis/open-and-closed-balls-in-a-metric-space|closed ball]] in $X$ of center $x_0$ and radius $r$ is the set
$$
B'(x_0;r):=\{x\in X\mid d(x,x_0)\le r\}.
$$

**Definition 1.33.** A subset $A$ of a metric space $X$ is [[convex-analysis/open-subset|open]] if for each $a\in A$, there exists an open ball $B(a;\delta)$ such that $B(a;\delta)\subset A$.

**Proposition 1.34.** ([[convex-analysis/open-balls-are-open-sets|Open balls are open sets]]) In any metric space, an open ball is an open set.

**Theorem 1.35.** ([[convex-analysis/basic-properties-of-open-sets|Basic properties of open sets]]) Let $X$ be a metric space. The following hold:
...
**Definition 1.36.** A subset $F$ of a metric space $X$ is called [[convex-analysis/closed-subset|closed]] if its complement $F^c=X\setminus F$ is open.

**Proposition 1.37.** ([[convex-analysis/closed-balls-are-closed-sets|Closed balls are closed sets]]) In any metric space, a closed ball is a closed set.

**Proposition 1.38.** ([[convex-analysis/basic-properties-of-closed-sets|Basic properties of closed sets]]) Let $X$ be a metric space. The following hold:
...
**Definition 1.39.** Let $E$ be a subset of a metric space $X$. The [[convex-analysis/interior-of-a-set|interior]] of $E$, denoted by $\operatorname{int}(E)$ or $E^\circ$, is defined as the union of all open sets contained in $E$:
$$
...
**Proposition 1.41.** ([[convex-analysis/basic-properties-of-the-interior-operator|Basic properties of the interior operator]]) Let $A,B\subset X$, where $X$ is a metric space. Then:
...
**Proposition 1.42.** ([[convex-analysis/interior-characterized-by-existence-of-a-ball|Interior characterized by existence of a ball]]) Let $X$ be a metric space, let $a\in X$, and let $E\subset X$. Then
$$
...
**Definition 1.43.** Let $E$ be a subset of a metric space $(X,d)$. The [[convex-analysis/closure-of-a-set|closure]] of $E$, denoted by $\overline{E}$, is the intersection of all closed sets that contain $E$.
...
**Proposition 1.45.** ([[convex-analysis/basic-properties-of-the-closure-operator|Basic properties of the closure operator]]) Let $A,B\subset X$. Then:
...
**Proposition 1.46.** ([[convex-analysis/closure-characterized-by-ball-intersections|Closure characterized by ball intersections]]) Let $(X,d)$ be a metric space and let $E$ be a subset of $X$. For any element $a\in X$, the following are equivalent:
...
**Definition 1.47.** Let $(X,d)$ be a metric space. A sequence $(x_n)$ of points in $X$ is said to [[convex-analysis/convergence-of-a-sequence|converge]] to a point $a\in X$ if for each $\varepsilon>0$, there exists $N\in\mathbb{N}$ such that
$$
...
**Proposition 1.49.** ([[convex-analysis/uniqueness-of-limits-in-metric-spaces|Uniqueness of limits]]) A sequence $(x_n)$ in a metric space $X$ has at most one limit.
...
**Definition 1.50.** A subset $A$ of a metric space $X$ is [[convex-analysis/bounded-set-and-bounded-sequence|bounded]] if there exists a ball $B(a;r)$ such that $A\subset B(a;r)$. Therefore, a sequence $(x_n)$ is bounded if the set $\{x_n\mid n\in\mathbb{N}\}$ is bounded.

**Proposition 1.51.** ([[convex-analysis/convergent-sequences-are-bounded|Convergent sequences are bounded]]) Any convergent sequence in a metric space is bounded.
...
**Proposition 1.52.** ([[convex-analysis/closure-characterized-by-convergent-sequences|Closure characterized by convergent sequences]]) Let $X$ be a metric space and let $A$ be a subset of $X$. Then
$$
...
**Proposition 1.53.** ([[convex-analysis/closed-sets-characterized-by-sequences-version-i|Closed sets characterized by sequences]]) Let $A$ be a subset of a metric space $X$. Then $A$ is closed if and only if whenever $(a_n)$ is a sequence in $A$ that converges to $a$, we have $a\in A$.
...
**Definition 1.55 (Cauchy sequence).** Let $(X,d)$ be a metric space. A sequence $(x_n)$ in $X$ is called a [[topology/cauchy-sequence|Cauchy sequence]] if for each $\varepsilon>0$, there exists $N\in\mathbb{N}$ such that $d(x_m,x_n)<\varepsilon$ for all $m,n\ge N$.

**Proposition 1.56.** ([[convex-analysis/convergent-sequences-are-cauchy|Convergent sequences are Cauchy]]) If $(x_n)$ is a convergent sequence in a metric space, then it is a Cauchy sequence.
...
**Proposition 1.57.** ([[convex-analysis/cauchy-sequences-are-bounded|Cauchy sequences are bounded]]) A Cauchy sequence in a metric space is bounded.
...
**Lemma 1.60.** ([[convex-analysis/subsequence-index-bound-n_k-k|Subsequence index bound]]) Let $(n_k)_k$ be a sequence of positive integers with $n_1<n_2<\cdots$. Then $n_k\ge k$ for all $k=1,2,\dots$.
...
**Proposition 1.61.** ([[convex-analysis/subsequences-of-convergent-sequences-converge-to-the-same-limit|Subsequences of convergent sequences converge to the same limit]]) If a sequence $(x_n)$ converges to $a$, then any subsequence $(x_{n_k})$ of $(x_n)$ also converges to $a$.
...
**Proposition 1.62.** ([[convex-analysis/cauchy-sequence-with-a-convergent-subsequence-converges|Cauchy sequence with a convergent subsequence converges]]) A Cauchy sequence in a metric space that has a convergent subsequence is convergent.
...
**Definition 1.63 (complete metric spaces).** A metric space $(X,d)$ is said to be [[convex-analysis/complete-metric-space-complete-subset|complete]] if every Cauchy sequence in $X$ is convergent. That means: if $(x_n)$ is a Cauchy sequence in $X$, then there exists $x\in X$ such that $\lim_{n\to\infty} x_n=x$.

A subset $E\subset X$ is called **complete** if $(E,d)$ is a complete metric space.

**Proposition 1.64.** ([[convex-analysis/completeness-implies-closedness-closed-subsets-of-complete-spaces-are-complete|Completeness and closedness]]) Let $X$ be a metric space and let $E\subset X$. Then:

(i) If $E$ is complete, then it is closed.
(ii) If $X$ is complete and $E$ is closed, then $E$ is complete.

**Theorem 1.65.** ([[convex-analysis/completeness-of-rk|Completeness of R^k]]) $\mathbb{R}^k$ is complete for any positive integer $k$.
...
**Definition 1.66.** Let $X$ be a vector space over a field $K$ (either $\mathbb{R}$ or $\mathbb{C}$). A function $\|\cdot\|:X\to\mathbb{R}$ is called a [[convex-analysis/norm-normed-vector-space|norm]] on $X$ if the following conditions hold for all $x,y\in X$ and $\lambda\in K$:

(i) $\|x\|\ge 0$, and $\|x\|=0$ if and only if $x=0$.
(ii) $\|\lambda x\|=|\lambda|\,\|x\|$.
(iii) $\|x+y\|\le \|x\|+\|y\|$ (the triangle inequality).

If $\|\cdot\|$ is a norm on $X$, then $(X,\|\cdot\|)$ is called a [[convex-analysis/norm-normed-vector-space|normed vector space]] (or a **normed linear space**) over $K$. We can also say simply that $X$ is a normed space over $K$ if no confusion occurs. If the field $K$ is $\mathbb{R}$ (resp. $\mathbb{C}$), then we say that $X$ is a real (resp. complex) normed space. The real number $\|x\|$ is called the **norm** of the vector $x\in X$.

**Proposition 1.67.** ([[convex-analysis/norm-induces-a-metric-and-conversely|Norm induces a metric and conversely]]) Let $X$ be a normed space over $K$. Define $d:X\times X\to\mathbb{R}$ by
$$
...
**Proposition 1.68.** ([[real-analysis/reverse-triangle-inequality|Reverse triangle inequality]]) Let $(X,\|\cdot\|)$ be a normed space. Then
$$
...
**Definition 1.73.** Let $X$ be a normed space and let $(x_n)$ be a sequence in $X$. The sequence $(x_n)$ is said to [[convex-analysis/convergence-in-normed-spaces|converge]] in $X$ if there exists $x\in X$ such that $\|x_n-x\|\to 0$ as $n\to\infty$.
...
**Proposition 1.77.** ([[convex-analysis/convergence-implies-convergence-of-norms|Convergence implies convergence of norms]]) Consider a normed space $X$ over a field $K$. Let $(x_n)$ be a sequence in $X$. If $x_n\to x_0$ in $X$, then $\|x_n\|\to \|x_0\|$ in $\mathbb{R}$.
...
**Proposition 1.78.** ([[convex-analysis/uniqueness-of-limits-and-boundedness-in-normed-spaces|Uniqueness of limits and boundedness]]) Consider a normed space $X$ over a field $K$. Let $(x_n)$ be a sequence in $X$. Then:
...
**Proposition 1.79.** ([[convex-analysis/algebra-of-limits-in-normed-spaces|Algebra of limits in normed spaces]]) Consider a normed space $X$ over a field $K$. Let $(x_n)$, $(y_n)$ be sequences in $X$ and let $(\alpha_n)$ be a sequence in $K$. Then:
...
**Definition 2.1.** A subset $\Omega$ of a vector space $X$ is called [[convex-analysis/convex-set|convex]] if we have
$$
\lambda x + (1-\lambda)y \in \Omega \quad \text{for all } x,y\in \Omega \text{ and } \lambda\in(0,1).
$$

Given $a,b\in X$, the [[convex-analysis/line-segments-in-a-vector-space|line segment]] $[a,b]\subset X$ connecting these points is
$$
...
Let $X$ and $Y$ be vector spaces. A mapping $B:X\to Y$ is called [[convex-analysis/affine-mapping|affine]] if there exist a linear mapping $A:X\to Y$ and a vector $b\in Y$ such that
$$
B(x)=A(x)+b \quad \text{for all } x\in X. \qquad (2.2)
$$
The following proposition provides a characterization of affine mappings.

**Proposition 2.2.** ([[convex-analysis/characterization-of-affine-mappings|Characterization of affine mappings]]) Let $X$ and $Y$ be vector spaces. Then $B:X\to Y$ is an affine mapping if and only if we have
$$
...
**Proposition 2.3.** ([[convex-analysis/affine-images-and-preimages-of-convex-sets-are-convex|Affine images and preimages of convex sets are convex]]) Let $X$ and $Y$ be vector spaces, and let $B:X\to Y$ be an affine mapping. The following assertions hold:
...
**Proposition 2.4.** ([[convex-analysis/cartesian-product-of-convex-sets-is-convex|Cartesian product of convex sets is convex]]) Let $X$ and $Y$ be vector spaces. If $\Omega_1$ is a convex subset of $X$ and $\Omega_2$ is a convex subset of $Y$, then the Cartesian product $\Omega_1\times \Omega_2$ is a convex subset of the product space $X\times Y$.
...
**Proposition 2.5.** ([[convex-analysis/sums-and-scalar-multiples-of-convex-sets-are-convex|Sums and scalar multiples of convex sets are convex]]) Let $\Omega_1$ and $\Omega_2$ be convex subsets of $X$, and let $\lambda\in\mathbb{R}$ be a scalar. Then $\lambda\Omega_1$ and $\Omega_1+\Omega_2$ are convex subsets of $X$.
...
A vector $x\in X$ is called a [[convex-analysis/convex-combination|convex combination]] of $x_1,\dots,x_m\in X$ if there are numbers $\lambda_1,\dots,\lambda_m\ge 0$ such that
$$
...
**Proposition 2.6.** ([[convex-analysis/convex-sets-characterized-by-closure-under-convex-combinations|Convex sets characterized by closure under convex combinations]]) A subset $\Omega$ of a vector space $X$ is convex if and only if it contains all the convex combinations of its elements.
...
**Proposition 2.7.** ([[convex-analysis/intersections-of-convex-sets-are-convex|Intersections of convex sets are convex]]) Let $\{\Omega_\alpha\}_{\alpha\in I}$ be a collection of convex subsets of $X$. Then $\bigcap_{\alpha\in I}\Omega_\alpha$ is also a convex subset of $X$.
...
**Definition 2.8.** Let $\Omega$ be a subset of a vector space $X$. The [[convex-analysis/convex-hull|convex hull]] of $\Omega$ is the intersection of all convex sets in $X$ that contain $\Omega$, i.e.,
$$
...
**Proposition 2.9.** ([[convex-analysis/convex-hull-is-the-smallest-convex-set-containing|Convex hull is the smallest convex set containing]]) Let $X$ be a vector space, and let $\Omega$ be a subset of $X$. Then the convex hull $\operatorname{co}(\Omega)$ is the smallest convex set containing $\Omega$.
...
**Theorem 2.10.** ([[convex-analysis/convex-hull-via-convex-combinations|Convex hull via convex combinations]]) For any subset $\Omega$ of a vector space $X$, its convex hull $\operatorname{co}(\Omega)$ admits the representation
$$
...
**Proposition 2.11.** ([[convex-analysis/interior-and-closure-of-a-convex-set-are-convex|Interior and closure of a convex set are convex]]) Let $X$ be a normed vector space. Then the interior $\operatorname{int}(\Omega)$ and closure $\overline{\Omega}$ of a convex set $\Omega\subset X$ are also convex.
...
**Theorem 2.13.** ([[convex-analysis/interior-and-closure-relations-for-convex-sets-with-nonempty-interior|Interior and closure relations for convex sets]]) Let $X$ be a normed vector space, and let $\Omega\subset X$ be a convex set with nonempty interior. Then the following assertions hold:
...
**Theorem 2.14.** ([[convex-analysis/closure-of-intersections-under-interior-point-condition|Closure of intersections under interior point condition]]) Let $X$ be a normed vector space, and let $\Omega_1$ and $\Omega_2$ be convex subsets of $X$ satisfying the qualification condition
$$
...
We consider the [[convex-analysis/extended-real-number-system-and-conventions|extended real number system]] which consists of $\mathbb{R}$ and two symbols $-\infty$ and $\infty$. Along with the usual properties of $\mathbb{R}$, the conventions of extended arithmetic and orders are as follows:
$$
\begin{aligned}
...
Given such a function $f:X\to \overline{\mathbb{R}}$, let us associate with it the [[convex-analysis/domain-and-epigraph-proper-function|domain and epigraph]], which are the sets defined by
$$
...
**Theorem 2.17.** ([[convex-analysis/equivalent-characterizations-of-convex-functions|Equivalent characterizations of convex functions]]) The convexity of a function $f:X\to \overline{\mathbb{R}}$ on a vector space $X$ is equivalent to each of the following statements:
...
We say that $f$ is [[convex-analysis/convexity-on-a-convex-subset-via-extension|convex on]] $\Omega$ if its extension $\tilde f:X\to \overline{\mathbb{R}}$ is a convex function on $X$. It is easy to see that $f$ is convex on $\Omega$ if and only if for all $x,y\in\Omega$ and $\lambda\in(0,1)$ we have
$$
f\big(\lambda x+(1-\lambda)y\big)\le \lambda f(x)+(1-\lambda)f(y).
$$
...
**Corollary 2.18.** ([[convex-analysis/domain-of-a-convex-function-is-convex|Domain of a convex function is convex]]) Any convex function $f:X\to \overline{\mathbb{R}}$ defined on a vector space $X$ has a convex domain $\operatorname{dom}(f)$.
...
**Example 2.20.** Let $H$ be an inner product (in particular, a Hilbert) space. Recall that a linear operator $A:H\to H$ is called [[convex-analysis/self-adjoint-linear-operator|self-adjoint]] if
$$
\langle Ax,y\rangle = \langle x,Ay\rangle \quad \text{for all } x,y\in H.
$$

We say that $A$ is [[convex-analysis/nonnegative-positive-semidefinite-operator|nonnegative (or positive-semidefinite)]] if it is self-adjoint and $\langle Ax,x\rangle \ge 0$ for all $x\in H$.
...
**Example 2.21.** Let $\Omega$ be a nonempty subset of a vector space $X$. Associate with it the [[convex-analysis/indicator-function-of-a-set|indicator function]] $\delta_\Omega:X\to\overline{\mathbb{R}}$ by
$$
...
**Example 2.22.** Let $\Omega$ be a nonempty subset of a normed space $X$. Define the [[convex-analysis/distance-function-to-a-set|distance function]] $d_\Omega:X\to\mathbb{R}$ associated with $\Omega$ by
$$
...
**Definition 2.23.** ([[convex-analysis/strictly-convex-function|Strictly convex function]]) Let $f:X\to\overline{\mathbb{R}}$ be a function defined on a vector space $X$. It is said to be **strictly convex** if
$$
...
**Definition 2.25.** ([[convex-analysis/quasiconvex-function|Quasiconvex function]]) A function $f:X\to\overline{\mathbb{R}}$ defined on a vector space $X$ is called **quasiconvex** if we have
$$
...
**Proposition 2.26.** ([[convex-analysis/quasiconvexity-characterized-by-convex-sublevel-sets|Quasiconvexity characterized by convex sublevel sets]]) Let $X$ be a vector space. A function $f:X\to\overline{\mathbb{R}}$ is quasiconvex if and only if the sublevel set
$$
...
**Lemma 2.27.** ([[convex-analysis/slope-inequalities-for-convex-functions|Slope inequalities for convex functions]]) Let $f:I\to\mathbb{R}$ be a convex function, where $I\subset\mathbb{R}$ is a nonempty interval. Then for any different numbers $a,b\in I$ with $a<b$ and any $x\in(a,b)$ we have the inequalities
$$
...
**Corollary 2.29.** ([[convex-analysis/convexity-via-nonnegative-second-derivative|Convexity via nonnegative second derivative]]) Let $f:I\to\mathbb{R}$ be twice differentiable, where $I\subset\mathbb{R}$ is a nonempty open interval. Then $f$ is convex if and only if its second derivative is nonnegative on $I$, i.e., $f''(x)\ge 0$ for all $x\in I$.
...
**Theorem 2.30.** ([[convex-analysis/convexity-characterized-by-positive-semidefinite-hessian|Convexity characterized by positive-semidefinite Hessian]]) Let $f:\Omega\to\mathbb{R}$ be twice continuously differentiable on a nonempty open convex set $\Omega\subset\mathbb{R}^n$. Then the function $f$ is convex on $\Omega$ if and only if for all $x\in\Omega$ its Hessian matrix $\nabla^2 f(x)$ is positive-semidefinite, i.e., we have
$$
...
**Proposition 2.32.** ([[convex-analysis/weighted-arithmeticgeometric-mean-inequality|Weighted arithmetic-geometric mean inequality]]) For every $a,b\ge 0$ and $0<\theta<1$ we have the inequality
$$
...
**Proposition 2.33.** ([[convex-analysis/holder-inequality-finite-sums|Hölder inequality for finite sums]]) Let $x_i,y_i\in\mathbb{R}$ for $i=1,\dots,m$. Then for $p>1$ and $q>1$ such that $1/p+1/q=1$ we have the inequality
$$
\sum_{i=1}^m |x_i y_i|
...
**Proposition 2.34.** ([[convex-analysis/holder-inequality-integrals|Hölder inequality for integrals]]) Let $f,g:\mathbb{R}\to\mathbb{R}$ be summable functions of the corresponding degree on $[a,b]$, and let $\gamma(\cdot)$ stand for the Lebesgue measure on this interval. Given $p>1$ and $q>1$ with $1/p+1/q=1$, we have the inequality
$$
\int_a^b |fg|\,d\gamma
\le
...
**Proposition 2.35.** ([[convex-analysis/youngs-inequality|Young's inequality]]) Let $p,q>0$ be such numbers that $1/p+1/q=1$. Then we have the estimate
$$
...
**Proposition 2.36.** ([[convex-analysis/basic-operations-preserving-convexity|Basic operations preserving convexity]]) Let $X$ be a vector space, and let $f,f_i:X\to\overline{\mathbb{R}}$ be convex functions for all $i=1,\dots,m$. Then the following functions are convex as well:
...
**Proposition 2.38.** ([[convex-analysis/convexity-preserved-under-affine-composition|Convexity preserved under affine composition]]) Let $B:X\to Y$ be an affine mapping between vector spaces, and let $f:Y\to\overline{\mathbb{R}}$ be a convex function on $Y$. Then the composition $f\circ B$ is convex on $X$.
...
**Proposition 2.39.** ([[convex-analysis/supremum-of-convex-functions-is-convex|Supremum of convex functions is convex]]) Let $X$ be a vector space, and let $f_i:X\to\overline{\mathbb{R}}$ for $i\in I$ be a collection of convex functions with a nonempty index set $I$. Then the supremum function $f(x):=\sup_{i\in I} f_i(x)$ is convex on $X$.
...
**Definition 2.40.** ([[convex-analysis/marginal-optimal-value-function|Marginal / optimal value function]]) Given $F:X\rightrightarrows Y$ and $\varphi:X\times Y\to\overline{\mathbb{R}}$, the **optimal value** (or **marginal**) function associated with $F$ and $\varphi$ is defined by
$$
...
**Theorem 2.41.** ([[convex-analysis/convexity-of-the-marginal-optimal-value-function|Convexity of the marginal optimal value function]]) Let $X$ and $Y$ be vector spaces. Assume that $\varphi:X\times Y\to\overline{\mathbb{R}}$ is a convex function and that $F:X\rightrightarrows Y$ is a convex set-valued mapping. Then the optimal value function $\mu$ in (2.14) is convex.
...
Given a subset $\Omega$ of a real vector space $X$, define the following algebraic notions, known as the [[convex-analysis/algebraic-interior-core|algebraic interior (or core)]] of $\Omega$ and the [[convex-analysis/linear-closure|linear closure]] of $\Omega$, respectively, by
...
where $[w,x)$ is the [[convex-analysis/line-segments-in-a-vector-space|line segment]] connecting $w$ and $x$, excluding $x$.

Note that, if $\operatorname{int}(\Omega)$ denotes the [[convex-analysis/interior-of-a-set|interior]] of $\Omega$ and $\overline{\Omega}$ its [[convex-analysis/closure-of-a-set|closure]], then the following relationships hold:
...
**Proposition 3.1.** ([[convex-analysis/core-characterized-by-absorbing-translations|Core characterized by absorbing translations]]) Let $\Omega$ be a nonempty subset of a vector space $X$. Then $\bar{x} \in \operatorname{core}(\Omega)$ if and only if $\Omega - \bar{x}$ is an [[convex-analysis/balanced-and-absorbing-sets|absorbing set]].

**Proposition 3.2.** ([[convex-analysis/core-of-a-convex-set-is-convex|Core of a convex set is convex]]) The core of every [[convex-analysis/convex-set|convex]] subset of a vector space $X$ is also a convex subset of $X$.
...
**Theorem 3.3.** ([[convex-analysis/core-equals-interior-for-convex-sets-in-normed-spaces|Core equals interior for convex sets in normed spaces]]) Let $X$ be a [[convex-analysis/norm-normed-vector-space|normed vector space]], and let $\Omega \subset X$ be a convex set with nonempty interior. Then
...
**Proposition 3.5.** ([[convex-analysis/linear-closure-equals-closure-for-solid-convex-sets|Linear closure equals closure for solid convex sets]]) Let $X$ be a normed vector space, and let $\Omega$ be a convex subset of $X$ with nonempty interior. Then we have
...
**Proposition 3.6.** ([[convex-analysis/segments-from-core-points-stay-in-the-core|Segments from core points stay in the core]]) Let $\Omega$ be a convex set in a vector space $X$ and let $a \in \operatorname{core}(\Omega)$. Then
...
**Corollary 3.7.** ([[convex-analysis/idempotence-of-the-core-operator|Idempotence of the core operator]]) For any convex set $\Omega$ in a vector space $X$ we have
...
Let $\Omega$ be a nonempty [[convex-analysis/balanced-and-absorbing-sets|absorbing set]] in a vector space $X$. The [[convex-analysis/minkowski-function-gauge-of-a-set|Minkowski function]] $p_\Omega: X \to \overline{\mathbb{R}}$, known also as the Minkowski gauge, is defined by
...
**Definition 3.8.** ([[convex-analysis/subadditive-positively-homogeneous-and-sublinear-functions|Subadditive, positively homogeneous, and sublinear functions]]) Let $f : X \to \mathbb{R}$ be a function defined on a vector space $X$.
...
**Theorem 3.9.** ([[convex-analysis/properties-of-the-minkowski-functional-of-a-convex-set|Properties of the Minkowski functional of a convex set]]) Let $\Omega$ be an [[convex-analysis/balanced-and-absorbing-sets|absorbing]] [[convex-analysis/convex-set|convex set]] in a vector space $X$. Then the Minkowski function $p_\Omega$ has the following properties:
...
**Corollary 3.10.** ([[convex-analysis/continuity-and-level-sets-of-the-minkowski-functional|Continuity and level sets of the Minkowski functional]]) Let $X$ be a normed vector space, and let $\Omega$ be a convex set such that $0 \in \operatorname{int}(\Omega)$. Then $p_\Omega$ is continuous, and we have
...
Let $X$ be a vector space, let $Y$ be a [[convex-analysis/linear-subspace|linear subspace]] of $X$, and let $f$ be a linear functional defined on $Y$ with some restraint. The Hahn-Banach theorems allow us to extend $f$ to a linear functional $F$ defined on the whole space $X$ such that $F$ also satisfies the restraint as for $f$.

#### 3.2.1 Hahn-Banach theorem in real vector spaces

**Theorem 3.11.** ([[convex-analysis/hahn-banach-theorem-in-real-vector-spaces|Hahn-Banach theorem in real vector spaces]]) Let $X$ be a real [[linear-algebra/vector-space|vector space]], let $Y$ be a subspace of $X$, and let $p: X \to \mathbb{R}$ be a [[convex-analysis/subadditive-positively-homogeneous-and-sublinear-functions|sublinear function]] on $X$. Consider a linear functional $f : Y \to \mathbb{R}$ which satisfies the condition
...
**Definition 3.12.** ([[convex-analysis/seminorm|Seminorm]]) A function $p : X \to \mathbb{R}$ defined on a vector space $X$ over a field $K$ (either $\mathbb{R}$ or $\mathbb{C}$) is called a **seminorm** if
...
**Theorem 3.13.** ([[convex-analysis/hahn-banach-extension-dominated-by-a-seminorm-real-case|Hahn-Banach extension dominated by a seminorm (real case)]]) Let $X$ be a real vector space, let $Y$ be a linear subspace of $X$, and let $p : X \to \mathbb{R}$ be a seminorm on $X$. Let $f : Y \to \mathbb{R}$ be a linear functional which satisfies the condition:
...
**Theorem 3.14.** ([[convex-analysis/hahn-banach-theorem-in-complex-vector-spaces|Hahn-Banach theorem in complex vector spaces]]) Let $X$ be a complex vector space, let $Y$ be a linear subspace of $X$, and let $p : X \to \mathbb{R}$ be a seminorm on $X$. Let $f : Y \to \mathbb{C}$ be a complex linear functional which satisfies the condition:
...
Let $X$ be a [[convex-analysis/norm-normed-vector-space|normed space]] over the field $K$ ($\mathbb{R}$ or $\mathbb{C}$). Then $K$ is also a normed space where the norm is defined by $|\alpha| = |\alpha|$ (here $|\cdot|$ is the absolute value or module of a number in $K$). Remember that a [[convex-analysis/linear-operator-linear-transformation|linear operator]] from $X$ into $K$ is called a linear functional. Thus, a linear functional $f$ is [[convex-analysis/bounded-linear-functional-norm-of-a-functional|bounded]] if

$$
(\exists M>0)\ (\forall x \in X):\ |f(x)| \le M\|x\|.
$$

This condition holds iff $f$ is continuous on $X$. Then the [[convex-analysis/bounded-linear-functional-norm-of-a-functional|norm of $f$]] is defined by

$$
\|f\| = \sup_{\|x\|\le 1} |f(x)|.
$$

**Theorem 3.16.** ([[convex-analysis/hahn-banach-theorem-in-normed-spaces|Hahn-Banach theorem in normed spaces]]) Let $X$ be a normed space, let $Y$ be a subspace of $X$, and let $f : Y \to K$ be a bounded linear functional on $Y$. Then there exists a linear bounded functional $F$ defined on $X$ such that
...
**Theorem 3.17.** ([[convex-analysis/separation-of-a-point-and-a-subspace|Separating a point and a subspace]]) Let $Y$ be a subspace of a normed space $X$, let $x_0$ be a point in $X$ such that $d(x_0,Y) = \inf_{y\in Y}\|x_0 - y\| = d >0$. Then there exists a bounded linear functional on $X$ such that:
...
*Proof.* Since $d>0$, we see that $x_0 \notin Y$. Let $Z = Y \oplus \operatorname{span}\{x_0\}$ (the [[convex-analysis/direct-sum-of-subspaces|direct sum]] of $Y$ and the [[convex-analysis/subspace-generated-by-a-set-span|span]] of $x_0$). Then any $z \in Z$ can be expressed as $z = y + \lambda x_0$ for $y \in Y$, $\lambda \in K$. Note that $z \in Y$ iff $\lambda = 0$. Define a linear functional $g$ on $Z$ by
...
**Corollary 3.18.** ([[convex-analysis/existence-of-a-functional-attaining-its-norm-at-a-point|Existence of a functional attaining its norm at a point]]) For any element $z_0 \in X \setminus \{0\}$, there exists a bounded linear functional $f$ defined on $X$ such that $\|f\| = 1$ and $f(z_0) = \|z_0\|$.
...
The main goal of this subsection is to discuss some basic facts about [[convex-analysis/quotient-vector-space-codimension|quotient spaces]]. Given a linear subspace $L$ of a topological vector space $X$, recall that the quotient space $X/L$ is defined by
...
**Definition 3.20.** ([[convex-analysis/codimension|Codimension]]) Let $L$ be a linear subspace of a vector space $X$. The **codimension** of $L$ in $X$, denoted by $\operatorname{codim}(L)$, is the dimension of the quotient space $X/L$, i.e.,
...
**Proposition 3.21.** ([[convex-analysis/kernel-of-a-nonzero-linear-functional-has-codimension-one|Kernel of a nonzero linear functional has codimension one]]) Let $X$ be a vector space over a field $K$ (either $\mathbb{R}$ or $\mathbb{C}$), and let $f : X \to K$ be a nonzero linear function. Then we have

$$
\operatorname{codim}(\ker f) = 1.
$$

*Proof.* Denote $L := \ker(f)$ (the [[convex-analysis/image-and-kernel-linear-isomorphism|kernel]] of $f$) and fix $x_0 \in X$ with $f(x_0) = \alpha_0 \ne 0$. To show that $\operatorname{span}\{x_0 + L\} = X/L$, take $x \in X$ such that $x+L \in X/L$ and get $f(x) = \lambda f(x_0) = f(\lambda x_0)$ with $\lambda := \frac{f(x)}{f(x_0)}$. It follows that $f(x-\lambda x_0) = 0$, and so $x-\lambda x_0 \in L$. Thus we get the equality
...
**Proposition 3.22.** ([[convex-analysis/codimension-one-subspaces-yield-direct-sum-decompositions|Codimension one subspaces yield direct sum decompositions]]) Let $L$ be a subspace of a vector space $X$ such that $\operatorname{codim}(L) = 1$. If $x_0 \notin L$, then we have
...
Next we proceed with the definition and properties of [[convex-analysis/affine-set|affine sets]]; see the illustration in Figure 3.1. Given two elements $a$ and $b$ in a vector space $X$, the [[convex-analysis/line-connecting-two-points|line connecting them]] is defined by
...
**Definition 3.23.** ([[convex-analysis/affine-set|Affine set]]) Let $\Omega$ be a subset of a vector space $X$. We say that $\Omega$ is an **affine set** if for any $a,b \in \Omega$ we have $\mathcal{L}[a,b] \subset \Omega$.

The intersection of affine sets is affine, and so we can define the smallest affine set containing $\Omega$.

**Definition 3.24.** ([[convex-analysis/affine-hull-affine-combination|Affine hull and affine combination]]) The **affine hull** of a set $\Omega$ is defined by
...
**Proposition 3.25.** ([[convex-analysis/properties-of-affine-sets-and-affine-hulls|Properties of affine sets and affine hulls]]) The following assertions are true.
...
(iii) Let $B : X \to Y$ be an [[convex-analysis/affine-mapping|affine mapping]] between vector spaces $X$ and $Y$. If $\Omega$ is an affine subset of $X$ and $\hat{\Omega}$ is an affine subset of $Y$, then the image $B(\Omega)$ is an affine subset of $Y$ and the inverse image $B^{-1}(\hat{\Omega})$ is an affine subset of $X$.
...
**Lemma 3.26.** ([[convex-analysis/affine-sets-are-translates-of-subspaces|Affine sets are translates of subspaces]]) A nonempty subset $\Omega$ of a vector space $X$ is affine if and only if $\Omega - \omega$ is a linear subspace of $X$ for any $\omega \in \Omega$.
...
**Definition 3.27.** ([[convex-analysis/parallel-affine-set|Parallel affine set]]) An affine set $\Omega$ in a vector space $X$ is said to be **parallel** to a linear subspace $L \subset X$ if $\Omega = \omega + L$ for some $\omega \in \Omega$.

**Proposition 3.28.** ([[convex-analysis/parallel-subspace-to-an-affine-set-is|Parallel subspace to an affine set]]) Let $\Omega$ be a nonempty affine subset of a vector space $X$. Then it is parallel to the unique linear subspace $L$ of $X$ defined by $L := \Omega - \Omega$.
...
**Definition 3.29.** ([[convex-analysis/hyperplane|Hyperplane]]) An affine subset $\Omega$ of a vector space $X$ is called a **hyperplane** if its codimension is one. This means that the codimension of the unique linear subspace of $X$ parallel to $\Omega$ is one.

**Proposition 3.30.** ([[convex-analysis/hyperplanes-are-level-sets-of-nonzero-linear-functionals|Hyperplanes are level sets of nonzero linear functionals]]) A subset $\Omega$ of a vector space $X$ over $\mathbb{R}$ is a hyperplane if and only if there exist a nonzero linear function $f : X \to \mathbb{R}$ and a number $\alpha \in \mathbb{R}$ that provide the representation
...
**Definition 3.31.** ([[convex-analysis/separation-by-a-hyperplane|Separation by a hyperplane]]) Let $\Omega_1$ and $\Omega_2$ be two nonempty sets in a real vector space $X$. We say that $\Omega_1$ and $\Omega_2$ can be **separated by a hyperplane** if there exists a nonzero linear functional $f \in X' \setminus \{0\}$ such that
...
**Theorem 3.34.** ([[convex-analysis/separation-of-a-point-from-a-convex-set-via-the-core|Separation of a point from a convex set via the core]]) Let $X$ be a real vector space, let $x_0 \in X$, and let $\Omega$ be a [[convex-analysis/convex-set|convex set]] in $X$. Suppose that $\operatorname{core}\Omega \ne \emptyset$ and $x_0 \notin \operatorname{core}\Omega$. Then the sets $\Omega$ and $\{x_0\}$ can be separated by a hyperplane, i.e., there exists $f \in X'\setminus\{0\}$ such that

$$
f(x) \le f(x_0) \quad \text{for all } x \in \Omega.
$$

*Proof.* First consider the case where $0 \in \operatorname{core}\Omega$. Let $p_\Omega$ be the [[convex-analysis/minkowski-function-gauge-of-a-set|Minkowski function]] associated with $\Omega$ and consider the linear subspace $Y = \operatorname{span}\{x_0\}$ of $X$. Note that $x_0 \ne 0$ because $x_0 \notin \operatorname{core}\Omega$. Define the linear function $g : Y \to \mathbb{R}$ by $g(\lambda x_0) = \lambda$ for $\lambda \in \mathbb{R}$. Obviously, $g$ is a linear functional. Next, we will show that
...
By the [[convex-analysis/hahn-banach-theorem-in-real-vector-spaces|Hahn-Banach theorem]], there exists a linear function $f : X \to \mathbb{R}$ such that $f(y) = g(y)$ for all $y \in Y$ and
...
**Lemma 3.35.** ([[convex-analysis/auxiliary-separation-lemma-for-disjoint-convex-sets-with-nonempty-core|Auxiliary separation lemma for disjoint convex sets with nonempty core]]) Let $X$ be a real vector space and let $\Omega_1,\Omega_2$ be two nonempty convex sets in $X$. Suppose that $\operatorname{core}\Omega_1 \ne \emptyset$ and $\Omega_1 \cap \Omega_2 = \emptyset$. Then $\Omega_1$ and $\Omega_2$ can be separated by a hyperplane.
...
**Theorem 3.36.** ([[convex-analysis/separation-of-two-convex-sets-via-the-core-condition|Separation of two convex sets via the core condition]]) Let $X$ be a real vector space and let $\Omega_1,\Omega_2$ be two nonempty convex sets in $X$. Suppose that $\operatorname{core}\Omega_1 \ne \emptyset$ and $(\operatorname{core}\Omega_1) \cap \Omega_2 = \emptyset$. Then $\Omega_1$ and $\Omega_2$ can be separated by a hyperplane.
...
**Theorem 3.38.** ([[convex-analysis/continuity-of-linear-functionals-via-closed-level-sets|Continuity of linear functionals via closed level sets]]) Let $X$ be a real normed space, let $f : X \to \mathbb{R}$ be a nonzero linear functional on $X$, and let $\alpha \in \mathbb{R}$. Then $f$ is continuous if and only if the set
...
*Proof.* We only need to prove the sufficient condition. Suppose that $A$ is a closed set. Note that $A$ is a proper subset of $X$, so there exists $x_0 \in X\setminus A$. Choose $r>0$ such that the [[convex-analysis/open-and-closed-balls-in-a-metric-space|open ball]] $B(x_0;r) \subset X\setminus A$. Since $f(B(x_0;r))$ is a convex set, it is an interval in $\mathbb{R}$ that does not contain $\alpha$. Without loss of generality, we can assume that
...
Let $X$ be a normed space and let $X^\ast $ represent the set of all continuous linear functionals on $X$. For each $x^\ast  \in X^\ast $ and $x \in X$, the expression $\langle x^\ast ,x\rangle = x^\ast (x)$ defines the [[convex-analysis/dual-space-and-duality-pairing|duality pairing]] between $x^\ast $ and $x$.

**Definition 3.39.** ([[convex-analysis/separation-by-a-closed-hyperplane|Separation by a closed hyperplane]]) Let $\Omega_1$ and $\Omega_2$ be two nonempty sets in a real normed space $X$. We say that $\Omega_1$ and $\Omega_2$ can be **separated by a closed hyperplane** if there exists a nonzero continuous linear functional $x^\ast  \in X^\ast \setminus\{0\}$ such that
...
**Theorem 3.40.** ([[convex-analysis/separation-by-closed-hyperplane-under-interior-condition|Separation by closed hyperplane under interior condition]]) Let $X$ be a real normed space and let $\Omega_1,\Omega_2$ be two nonempty convex sets in $X$. Suppose that $\operatorname{int}\Omega_1 \ne \emptyset$ and $(\operatorname{int}\Omega_1)\cap \Omega_2 = \emptyset$. Then $\Omega_1$ and $\Omega_2$ can be separated by a closed hyperplane.
...
**Corollary 3.41.** ([[convex-analysis/strict-separation-with-an-open-convex-set|Strict separation with an open convex set]]) Let $X$ be a real normed space, and let $G,\Omega$ be two nonempty convex sets in $X$. Suppose that $G$ is [[convex-analysis/open-subset|open]]. Then there exists $x^\ast  \in X^\ast $ and $\beta \in \mathbb{R}$ such that

$$
\langle x^\ast ,x\rangle < \beta \le \langle x^\ast ,y\rangle \quad \text{whenever } x \in G,\, y \in \Omega.
$$

**Definition 3.42.** ([[convex-analysis/strict-separation-by-a-closed-hyperplane|Strict separation by a closed hyperplane]]) Let $\Omega_1$ and $\Omega_2$ be two nonempty sets in a real normed space $X$. We say that $\Omega_1$ and $\Omega_2$ can be **strictly separated by a closed hyperplane** if there exist $x^\ast  \in X^\ast $ and $\alpha,\beta \in \mathbb{R}$ such that
