...
Let $(X,d)$ be a [[convex-analysis/metric-metric-space|metric space]], and let $(x_n)$ be a sequence in $X$ that [[convex-analysis/convergence-of-a-sequence|converges]] to $a\in X$. If $(x_{n_k})$ is a [[real-analysis/subsequence|subsequence]] of $(x_n)$, then $x_{n_k}\to a$.
...
- In $\mathbb{R}$, if $x_n=1/n\to 0$, then any subsequence $x_{n_k}=1/n_k$ also converges to $0$.
