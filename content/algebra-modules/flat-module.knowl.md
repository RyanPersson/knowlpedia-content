...
\]
is exact, where \(\otimes_R\) is the [[algebra-modules/tensor-product|tensor product]].
...
Equivalently, the functor \(-\otimes_R M\) is exact (it is always right-exact, so flatness is precisely left-exactness).
