...
\]
natural in \(M,N,P\), where \(\otimes_R\) is the [[algebra-modules/tensor-product|tensor product]] and \(\mathrm{Hom}_R(N,P)\) is the [[algebra-modules/hom-module|Hom module]]. Concretely, a map \(f\colon M\otimes_R N\to P\) corresponds to the map \(\Phi(f)\colon M\to \mathrm{Hom}_R(N,P)\) defined by \(\Phi(f)(m)(n)=f(m\otimes n)\), with inverse \(\Psi(g)(m\otimes n)=g(m)(n)\).

This adjunction is a formal consequence of the [[algebra-modules/tensor-product-universal-property|universal property of the tensor product]]: it linearizes [[algebra-modules/bilinear-map|bilinear maps]], and it specializes to duality when \(P=R\), giving \(\mathrm{Hom}_R(M\otimes_R N,R)\cong \mathrm{Hom}_R(M,N^\vee)\) with \(N^\vee\) the [[algebra-modules/dual-module|dual module]]
