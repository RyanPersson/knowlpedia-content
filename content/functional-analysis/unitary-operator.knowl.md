...
spaces]]. A **unitary operator** is a surjective linear map \(U:H\to K\)
...
A unitary operator on \(H\) is normal, has operator norm \(1\) when
...
Multiplication by a measurable function \(u\) with \(|u|=1\) almost
...
For real Hilbert spaces, the analogous maps are normally called orthogonal
