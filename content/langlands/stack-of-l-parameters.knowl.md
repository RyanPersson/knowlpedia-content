...
Let \(E\) be a nonarchimedean local field, let \(W_E\) be its [[langlands/weil-group|Weil group]],
and let \(\widehat G\) carry the pinned \(W_E\)-action determined by a
reductive \(E\)-group \(G\). The **stack of local \(L\)-parameters** is the
quotient stack
...
\(W_E\to{}^LG\) whose projection to \(W_E\) is the identity.
...
Keeping this [[algebra-groups/automorphism-group|automorphism group]] is necessary for packet enhancements and
...
identified without qualification with the moduli of full Weil–Deligne
parameters.
...
perfect complexes on it act on sheaves on
