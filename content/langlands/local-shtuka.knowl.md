...
Frobenius-compatible modification of \(G\)-bundles at one or more untilt
divisors, with relative positions bounded by specified [[algebra-groups/conjugacy-class|conjugacy classes]] of
cocharacters.
...
\(K\subset G(E)\) is compact open.

## Geometric data

Equivalently, on the Frobenius cover \(Y_{S,E}\), one describes a
\(G\)-torsor with an isomorphism to its Frobenius pullback away from the
...
reflex fields of the legs. Its compactly supported cohomology therefore
...
local Shimura varieties, including Rapoport–Zink and local
Shimura-datum examples. General local shtuka spaces allow arbitrary
