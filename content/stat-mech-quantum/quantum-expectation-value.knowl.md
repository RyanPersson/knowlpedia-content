...
kind = "knowl"
summary = "For a density operator rho and observable A, the expectation is Tr(rho A)."
...
Let $\rho$ be a [[quantum-foundations/density-operator|density operator]] on a finite-dimensional Hilbert space and let $A$ be a self-adjoint element of the [[stat-mech-quantum/observable-algebra|observable algebra]]. The **expectation value** of $A$ in the state $\rho$ is
$$
\langle A\rangle_\rho=\operatorname{Tr}(\rho A).
$$
For a pure state $\rho=|\psi\rangle\langle\psi|$, this reduces to $\langle A\rangle_\rho=\langle\psi|A|\psi\rangle$.
