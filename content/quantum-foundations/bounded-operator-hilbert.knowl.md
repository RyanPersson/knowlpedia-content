...
In finite dimension (e.g. on a [[quantum-foundations/complex-hilbert-space-finite|complex Hilbert space]]), **every** [[linear-algebra/linear-map|linear operator]] is bounded.
...
An operator \(A\) is [[quantum-foundations/self-adjoint-operator-observable|self-adjoint]] when \(A=A^\ast\).
