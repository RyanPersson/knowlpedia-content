+++
id = "stat-mech/debye-model"
title = "Debye model"
kind = "knowl"
summary = "Phonon continuum approximation with a Debye cutoff; produces the low-temperature heat capacity law and the high-temperature Dulong–Petit limit."
aliases = ["debye-model", "Debye model"]
domains = ["stat-mech"]
legacy_source_path = "stat-mech/debye-model.md"
+++
...
Instead of $3N$ identical modes as in the [[stat-mech/einstein-solid|Einstein solid]], one uses a distribution of frequencies up to a cutoff $\omega_D$ chosen so that the total number of modes is $3N$.

Thermodynamic quantities are computed in the [[stat-mech/canonical-ensemble|canonical ensemble]] at temperature $T$.
...
For each mode of frequency $\omega$, the mean energy is that of a [[stat-mech/quantum-harmonic-oscillator-example|quantum harmonic oscillator]]:
$$
...
Differentiating $U(T)$ gives the Debye heat capacity (see [[thermodynamics/heat-capacity-constant-volume|heat capacity at constant volume]]):
$$
...
- High temperature $T\gg \Theta_D$:
  ...
  C_V(T)\to 3N k_B,
  $$
  recovering the Dulong–Petit limit (also captured by [[stat-mech/einstein-solid|Einstein solid]]).
