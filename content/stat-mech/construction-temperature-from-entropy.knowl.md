+++
id = "stat-mech/construction-temperature-from-entropy"
title = "Temperature from entropy"
kind = "knowl"
summary = "Defines thermodynamic temperature (and inverse temperature) through the derivative of entropy with respect to energy."
aliases = ["construction-temperature-from-entropy", "Temperature from entropy"]
domains = ["stat-mech"]
legacy_source_path = "stat-mech/construction-temperature-from-entropy.md"
+++
...
For an isolated system with energy $U$, volume $V$, and particle number $N$, the [[stat-mech/boltzmann-entropy-microcanonical|Boltzmann (microcanonical) entropy]] is
$$
S(U,V,N) \;=\; k_B \log \Omega(U,V,N),
$$

where $\Omega(U,V,N)$ is a density of states (or phase-space volume of a thin [[stat-mech/microcanonical-shell|microcanonical energy shell]]), as described in [[thermodynamics/construction-microcanonical-entropy-density-of-states|the entropy–density-of-states construction]].

## Definition of temperature and inverse temperature

The **thermodynamic temperature** (as in [[thermodynamics/temperature-thermo|thermodynamic temperature]]) is defined by
$$
\frac{1}{T}
\;=\;
\left(\frac{\partial S}{\partial U}\right)_{V,N}.
$$
Equivalently, the [[thermodynamics/inverse-temperature-beta|inverse temperature]] is
$$
\beta \;=\; \frac{1}{k_B T}
\;=\;
...
with $\beta$ fixed by the bath’s slope. This leads to the canonical weight $\exp(-\beta H)$ for subsystem (1), connecting directly to the [[stat-mech/canonical-ensemble|canonical ensemble]] and to [[stat-mech/construction-entropy-maximization-thermal|entropy-maximization construction of thermal states]].
