+++
id = "stat-mech/construction-reduced-density-matrix"
title = "Reduced density matrix (construction)"
kind = "knowl"
summary = "Subsystem state obtained by tracing out degrees of freedom; it encodes all local expectations and correlations of a quantum state."
aliases = ["construction-reduced-density-matrix", "Reduced density matrix (construction)"]
domains = ["stat-mech"]
legacy_source_path = "stat-mech/construction-reduced-density-matrix.md"
+++
...
A (mixed) quantum state is represented by a density operator $\rho$ on $\mathcal H$, which in finite dimensions is a positive semidefinite [[linear-algebra/matrix|matrix]] with [[linear-algebra/trace|trace]] equal to $1$:
$$
\rho \ge 0,
\qquad
...
Equivalently, choosing any orthonormal basis $\{|b\rangle\}$ of $\mathcal H_B$ (compatible with the [[linear-algebra/inner-product|inner product]]), one has the concrete formula
$$
...
This definition guarantees that local expectations computed using the reduced state agree with the global [[stat-mech/ensemble-average|ensemble average]].

## Reduced thermal state in the canonical ensemble

For a quantum Hamiltonian $H$ at inverse temperature $\beta$, the [[stat-mech/canonical-ensemble|canonical ensemble]] uses the thermal density matrix
$$
...
- **Entropy:** the von Neumann entropy
  ...
  $$
  is the quantum analogue of the entropy concepts discussed in [[stat-mech/gibbs-entropy-shannon|Gibbs/Shannon entropy]] and [[thermodynamics/thermodynamic-entropy|thermodynamic entropy]] (with important quantum-specific features due to entanglement).

Reduced density matrices are also the natural inputs for defining [[stat-mech/construction-quantum-thermal-correlation|quantum thermal correlation functions]] restricted to a subsystem.
