+++
id = "stat-mech/markov-semigroup-continuous"
title = "Markov semigroup (continuous time)"
kind = "knowl"
summary = "One-parameter family of Markov operators describing continuous-time stochastic evolution; characterized by a generator and linked to the master equation."
aliases = ["markov-semigroup-continuous", "Markov semigroup (continuous time)"]
domains = ["stat-mech"]
legacy_source_path = "stat-mech/markov-semigroup-continuous.md"
+++
...
Prerequisites: [[probability/probability-measure|probability measure]], [[probability/expectation|expectation]], [[stat-mech/master-equation|master equation]].
...
For pure jump processes on a countable state space, this becomes the [[stat-mech/master-equation|master equation]] with rate matrix $Q$.

## Detailed balance
A stationary $\pi$ satisfies [[stat-mech/detailed-balance|detailed balance]] (reversibility) when $L$ is self-adjoint in $L^2(\pi)$, equivalently when stationary probability currents vanish.
