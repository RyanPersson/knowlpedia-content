+++
id = "stat-mech/construction-mayer-expansion"
title = "Mayer expansion (construction)"
kind = "knowl"
summary = "Graph expansion of the grand partition function of a classical interacting gas in powers of activity, using the Mayer f-bond."
aliases = ["construction-mayer-expansion", "Mayer expansion (construction)"]
domains = ["stat-mech"]
legacy_source_path = "stat-mech/construction-mayer-expansion.md"
+++

The **Mayer expansion** is a classical, explicit instance of a [[stat-mech/construction-cluster-expansion|cluster expansion]] for continuum gases. It expands the [[stat-mech/grand-partition-function|grand partition function]] in powers of the activity $z$ and expresses $\log \Xi$ (hence the pressure) as a sum over **connected graphs** built from the Mayer f-function.

## Setup: grand canonical gas with pair interactions

Consider a classical gas in a bounded region $\Lambda\subset\mathbb{R}^d$ at inverse temperature $\beta$ (see [[thermodynamics/inverse-temperature-beta|inverse temperature $\beta$]]), described in the [[stat-mech/grand-canonical-ensemble|grand canonical ensemble]]. For a pair potential $\phi(x)$, the total interaction energy for $N$ particles at positions $x_1,\dots,x_N$ is
$$
...
$$
where the configuration integral is an ordinary (Lebesgue) multiple integral; the underlying measure-theoretic viewpoint is the [[measure-theory/lebesgue-integral|Lebesgue integral]].
...
The pressure is recovered from the grand partition function using [[stat-mech/pressure-from-partition-function|pressure from the partition function]]. Formally,
$$
\beta\,p(z,\beta)
={}
...
For physically reasonable pair potentials (typically **stable** and **tempered**) the Mayer series converges for sufficiently small activity $z$ (low density) and/or high temperature. In that regime, the connected-graph structure makes $\log\Xi_\Lambda$ extensive and compatible with the [[thermodynamics/thermodynamic-limit|thermodynamic limit]].
