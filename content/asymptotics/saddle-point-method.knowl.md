...
for large $n$, where $\Gamma$ is a contour in the complex plane. It is closely related to [[asymptotics/laplaces-method|laplaces-method]], but uses complex contours chosen to pass through a *saddle point* (a stationary point of the phase).
...
If one can reduce a problem to an integral with a large parameter and a single dominant stationary point, the saddle-point method is the complex analogue of [[asymptotics/laplaces-method|laplaces-method]]. Both rely on:
