+++
id = "knowlification/conjectures-new-knowls"
title = "New knowls for the conjectures collection"
kind = "index"
summary = "The definitions added while knowlifying twelve selected formal conjectures and open problems."
aliases = ["conjectures new knowls", "formal conjectures glossary"]
domains = ["knowlification", "conjectures"]
+++

This temporary review index lists every supporting knowl added for the conjecture collection. The open-problem pages themselves live in the separately mounted local `conjectures-catalog` content source.

## Topology, actions, and homotopy

- [[topology/locally-compact-space|Locally compact space]]
- [[topology/locally-compact-group|Locally compact group]]
- [[topology/continuous-group-action|Continuous group action]]
- [[topology/topological-manifold|Topological manifold]]
- [[topology/closed-manifold|Closed manifold]]
- [[topology/separable-space|Separable space]]
- [[topology/homotopy-equivalence|Homotopy equivalence]]
- [[topology/homotopy-sphere|Homotopy sphere]]

## Arithmetic and real algebraic geometry

- [[shared-foundations/divisibility|Divisibility]]
- [[shared-foundations/factorial|Factorial]]
- [[shared-foundations/p-adic-valuation|\(p\)-adic valuation]]
- [[shared-foundations/p-adic-integers|\(p\)-adic integers]]
- [[convex-analysis/semialgebraic-set|Semialgebraic set]]
- [[convex-analysis/piecewise-polynomial-function|Piecewise-polynomial function]]

## Normed and Hilbert-space geometry

- [[linear-algebra/unit-sphere|Unit sphere of a normed space]]
- [[linear-algebra/surjective-linear-isometry|Surjective linear isometry]]
- [[linear-algebra/orthonormal-basis|Orthonormal basis]]
- [[linear-algebra/closed-linear-subspace|Closed linear subspace]]
- [[linear-algebra/orthogonal-complement|Orthogonal complement]]
- [[linear-algebra/orthogonal-projection|Orthogonal projection]]
- [[linear-algebra/operator-invariant-subspace|Invariant subspace of an operator]]

## Quantum information

- [[quantum-foundations/normalized-state-vector|Normalized state vector]]
- [[quantum-foundations/rank-one-projector|Rank-one projector]]
- [[quantum-foundations/positive-semidefinite-operator|Positive semidefinite operator]]
- [[quantum-foundations/positive-operator-valued-measure|Positive operator-valued measure]]
- [[quantum-foundations/normal-operator|Normal operator]]

## Complex geometry and group theory

- [[differential-geometry/holomorphic-map|Holomorphic map]]
- [[differential-geometry/complex-manifold|Complex manifold]]
- [[differential-geometry/almost-complex-structure|Almost-complex structure]]
- [[differential-geometry/integrable-almost-complex-structure|Integrable almost-complex structure]]
- [[algebra-groups/torsion-free-group|Torsion-free group]]

## Triangulated-category foundations

- [[algebra-category-theory/preadditive-category|Preadditive category]]
- [[algebra-category-theory/initial-object|Initial object]]
- [[algebra-category-theory/zero-object|Zero object]]
- [[algebra-category-theory/shift-functor|Shift functor]]
- [[algebra-category-theory/triangle|Triangle in a shifted category]]
- [[algebra-category-theory/distinguished-triangle|Distinguished triangle]]
- [[algebra-category-theory/pretriangulated-category|Pretriangulated category]]
- [[algebra-category-theory/octahedral-axiom|Octahedral axiom]]
- [[algebra-category-theory/triangulated-category|Triangulated category]]
- [[algebra-homological/mapping-cone|Mapping cone]]

## Important pre-existing knowls reused

- Topology and manifolds: [[topology/topological-space|topological space]], [[topology/continuous-map|continuous map]], [[topology/homeomorphism|homeomorphism]], [[topology/metric-sphere|metric sphere]], [[fiber-bundles/smooth-manifold|smooth manifold]], [[fiber-bundles/diffeomorphism|diffeomorphism]], and [[fiber-bundles/lie-group|Lie group]].
- Algebra and arithmetic: [[algebra-groups/group|group]], [[algebra-groups/faithful-action|faithful action]], [[algebra-fields-galois/galois-extension|Galois extension]], [[algebra-fields-galois/galois-group|Galois group]], [[algebra-representation-theory/group-algebra|group algebra]], and [[algebra-rings/zero-divisor|zero divisor]].
- Analysis and quantum theory: [[linear-algebra/banach-space|Banach space]], [[linear-algebra/hilbert-space|Hilbert space]], [[quantum-foundations/bounded-operator-hilbert|bounded operator]], and [[quantum-foundations/density-operator|density operator]].
- Categories: [[algebra-category-theory/category|category]], [[algebra-category-theory/morphism|morphism]], [[algebra-category-theory/additive-category|additive category]], and [[algebra-modules/short-exact-sequence|short exact sequence]].
