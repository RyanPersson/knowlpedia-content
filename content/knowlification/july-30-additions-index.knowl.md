+++
id = "knowlification/july-30-additions-index"
title = "July 30 mathematical knowl expansion"
kind = "index"
summary = "Review index for 372 new and 64 substantively refined production knowls in the July 30 mathematical expansion, organized by durable subject and dependency order."
aliases = ["July 30 additions", "July 30 mathematical expansion", "July 30 knowl review index"]
domains = ["knowlification", "algebra-hyperstructures", "formal-groups", "langlands", "complex-analysis", "differential-geometry", "mathematical-physics", "supergeometry"]
section_mode = "progressive"
prerequisites = []
dependency_heuristic = "semantic-full-review-v1"
dependency_review_count = 1
+++

This page is a review and navigation index for the July 30 mathematical
expansion. It lists every current new production knowl exactly once, followed
by the pre-existing knowls selected for substantive integration review. Purely
mechanical reference-placement updates elsewhere in the corpus are not repeated
in that second list. The lists follow durable mathematical subjects and place
foundations before their principal applications.

The atomicity re-audit folded broad survey-style records into subject maps or
replaced them with focused definitions, constructions, propositions, and
theorems. Folded records are therefore absent from the new-knowl count; their
surviving content is reached through the atomic entries below.

## New production knowls (372)

### Foundational topology and categorical algebra (16)

#### Shared foundations (1)

- [[shared-foundations/lattice|Lattice]]

#### Topology (2)

- [[topology/one-point-compactification|One-point compactification]]
- [[topology/simply-connected-space|Simply connected space]]

#### Categorical algebra (6)

- [[algebra-category-theory/algebra-object|Algebra object]]
- [[algebra-category-theory/core-of-a-category|Core of a category]]
- [[algebra-category-theory/group-object|Group object]]
- [[algebra-category-theory/groupoid|Groupoid]]
- [[algebra-category-theory/monoidal-category|Monoidal category]]
- [[algebra-category-theory/symmetric-monoidal-category|Symmetric monoidal category]]

#### Coalgebras, bialgebras, and Hopf algebras (3)

- [[algebra-coalgebras/bialgebra|Bialgebra]]
- [[algebra-coalgebras/coalgebra|Coalgebra]]
- [[algebra-coalgebras/hopf-algebra|Hopf algebra]]

#### Modules and algebra constructions (4)

- [[algebra-modules/category-of-finitely-generated-projective-modules|Category of finitely generated projective modules]]
- [[algebra-modules/exterior-algebra|Exterior algebra]]
- [[algebra-modules/symmetric-algebra|Symmetric algebra]]
- [[algebra-modules/tensor-algebra|Tensor algebra]]

### Rings, valuations, ordered algebra, and completions (30)

#### Commutative algebra (2)

- [[algebra-commutative/i-adic-completion|\(I\)-adic completion]]
- [[algebra-commutative/i-adic-topology|\(I\)-adic topology]]

#### Valuations and fields (4)

- [[algebra-fields-galois/non-archimedean-absolute-value|Non-Archimedean absolute value]]
- [[algebra-fields-galois/valuation-on-a-field|Valuation on a field]]
- [[algebra-fields-galois/valuation-ring|Valuation ring of a valued field]]
- [[algebra-fields-galois/value-group|Value group of a valuation]]

#### Ordered and projective groups (6)

- [[algebra-groups/commutative-monoid|Commutative monoid]]
- [[algebra-groups/lattice-ordered-abelian-group|Lattice-ordered abelian group]]
- [[algebra-groups/ordered-abelian-group|Ordered abelian group]]
- [[algebra-groups/pgl-psl-comparison|PGL–PSL comparison]]
- [[algebra-groups/projective-general-linear-group|Projective general linear group]]
- [[algebra-groups/projective-special-linear-group|Projective special linear group]]

#### Rings, semirings, and power series (15)

- [[algebra-rings/boolean-semifield|Boolean semifield]]
- [[algebra-rings/commutative-semiring|Commutative semiring]]
- [[algebra-rings/formal-inverse-function-theorem|Formal inverse function theorem]]
- [[algebra-rings/idempotent-semifield|Idempotent semifield]]
- [[algebra-rings/idempotent-semifields-and-lattice-ordered-groups|Idempotent semifields and lattice-ordered groups]]
- [[algebra-rings/idempotent-semiring|Idempotent semiring]]
- [[algebra-rings/multivariable-formal-power-series-ring|Multivariable formal power series ring]]
- [[algebra-rings/natural-order-of-idempotent-semiring|Natural order of an idempotent semiring]]
- [[algebra-rings/ordered-semiring|Ordered semiring]]
- [[algebra-rings/parasemifield|Parasemifield]]
- [[algebra-rings/semifield|Semifield]]
- [[algebra-rings/semiring|Semiring]]
- [[algebra-rings/semiring-homomorphism|Semiring homomorphism]]
- [[algebra-rings/substitution-of-formal-power-series|Substitution of formal power series]]
- [[algebra-rings/tropical-semifield|Tropical semifield]]

#### Topological algebra (3)

- [[algebra-topological/adic-ring|Adic ring]]
- [[algebra-topological/completed-tensor-product|Completed tensor product]]
- [[algebra-topological/ideal-of-definition|Ideal of definition]]

### Hyperstructures, semirings, and blueprint comparisons (34)

- [[algebra-hyperstructures|Algebra: Hyperstructures, semirings, and blueprints]]
- [[algebra-hyperstructures/band|Band]]
- [[algebra-hyperstructures/canonical-hypergroup|Canonical hypergroup]]
- [[algebra-hyperstructures/classification-of-stringent-hyperfields|Classification of stringent hyperfields]]
- [[algebra-hyperstructures/doubly-distributive-hyperfield|Doubly distributive hyperfield]]
- [[algebra-hyperstructures/doubly-distributive-hyperfields-are-stringent|Doubly distributive hyperfields are stringent]]
- [[algebra-hyperstructures/fields-semifields-and-hyperfields|Field embeddings into semifields and hyperfields]]
- [[algebra-hyperstructures/fusion-band|Fusion band]]
- [[algebra-hyperstructures/fusion-rule-for-bands|Fusion rule for bands]]
- [[algebra-hyperstructures/hyperfield|Hyperfield]]
- [[algebra-hyperstructures/hyperfield-as-a-tract|Tract associated with a hyperfield]]
- [[algebra-hyperstructures/hyperfield-of-a-field-quotient|Hyperfield of a field quotient]]
- [[algebra-hyperstructures/hyperoperation|Hyperoperation]]
- [[algebra-hyperstructures/hyperring|Hyperring]]
- [[algebra-hyperstructures/hyperring-homomorphism|Hyperring homomorphism]]
- [[algebra-hyperstructures/idyll|Idyll]]
- [[algebra-hyperstructures/integral-hyperring|Integral hyperring]]
- [[algebra-hyperstructures/krasner-hyperfield|Krasner hyperfield]]
- [[algebra-hyperstructures/null-set-of-a-band|Nullset of a band]]
- [[algebra-hyperstructures/partial-field|Partial field]]
- [[algebra-hyperstructures/partial-field-as-a-tract|Tract associated with a partial field]]
- [[algebra-hyperstructures/partial-hyperfield|Partial hyperfield]]
- [[algebra-hyperstructures/partial-hyperfield-as-a-tract|Tract associated with a partial hyperfield]]
- [[algebra-hyperstructures/pasture|Pasture]]
- [[algebra-hyperstructures/phase-hyperfield|Phase hyperfield]]
- [[algebra-hyperstructures/quotient-hyperring|Quotient hyperring]]
- [[algebra-hyperstructures/sign-hyperfield|Sign hyperfield]]
- [[algebra-hyperstructures/stringent-hyperfield|Stringent hyperfield]]
- [[algebra-hyperstructures/tract|Tract]]
- [[algebra-hyperstructures/tract-as-an-ordered-blueprint|Ordered blueprint associated with a tract]]
- [[algebra-hyperstructures/tropical-hyperfield|Tropical hyperfield]]
- [[algebra-hyperstructures/tropical-hyperfield-versus-semifield|Tropical semifield and hyperfield tied-sum distinction]]
- [[algebra-hyperstructures/valuation-as-tropical-hyperfield-morphism|Valuations as tropical hyperfield morphisms]]
- [[algebra-hyperstructures/valuative-hyperfield|Valuative hyperfield]]

### Algebraic and blue geometry (58)

- [[algebraic-geometry-foundations/affine-blue-scheme|Affine blue scheme]]
- [[algebraic-geometry-foundations/algebraic-curve|Algebraic curve]]
- [[algebraic-geometry-foundations/algebraic-group|Algebraic group]]
- [[algebraic-geometry-foundations/algebraic-space|Algebraic space]]
- [[algebraic-geometry-foundations/algebraic-stack|Algebraic stack]]
- [[algebraic-geometry-foundations/band-as-an-ordered-blueprint|Band as an ordered blueprint]]
- [[algebraic-geometry-foundations/blue-scheme|Blue scheme]]
- [[algebraic-geometry-foundations/blueprint|Blueprint]]
- [[algebraic-geometry-foundations/blueprint-as-an-ordered-blueprint|Blueprint as an ordered blueprint]]
- [[algebraic-geometry-foundations/borel-subgroup|Borel subgroup]]
- [[algebraic-geometry-foundations/coherent-sheaf|Coherent sheaf]]
- [[algebraic-geometry-foundations/commutative-monoid-with-zero-as-a-blueprint|Commutative monoid with zero as a blueprint]]
- [[algebraic-geometry-foundations/d-module|D-module]]
- [[algebraic-geometry-foundations/derived-algebraic-stack|Derived algebraic stack]]
- [[algebraic-geometry-foundations/finite-locally-free-morphism|Finite locally free morphism]]
- [[algebraic-geometry-foundations/flag-variety|Flag variety]]
- [[algebraic-geometry-foundations/formal-scheme|Formal scheme]]
- [[algebraic-geometry-foundations/formal-spectrum|Formal spectrum]]
- [[algebraic-geometry-foundations/fundamental-theorem-of-projective-geometry|Fundamental theorem of projective geometry]]
- [[algebraic-geometry-foundations/hyperrings-as-ordered-blueprints|Hyperrings as ordered blueprints]]
- [[algebraic-geometry-foundations/idyll-as-ordered-blue-field|Idyll as an ordered blue field]]
- [[algebraic-geometry-foundations/integrable-connection|Integrable algebraic connection]]
- [[algebraic-geometry-foundations/k-ideal-of-a-blueprint|\(k\)-ideal of a blueprint]]
- [[algebraic-geometry-foundations/localization-of-blueprint|Localization of a blueprint]]
- [[algebraic-geometry-foundations/locally-blueprinted-space|Locally blueprinted space]]
- [[algebraic-geometry-foundations/locally-free-sheaf|Locally free sheaf]]
- [[algebraic-geometry-foundations/locally-topologically-ringed-space|Locally topologically ringed space]]
- [[algebraic-geometry-foundations/moduli-stack-of-g-bundles-on-a-curve|Moduli stack of G-bundles on a curve]]
- [[algebraic-geometry-foundations/ordered-blue-field|Ordered blue field]]
- [[algebraic-geometry-foundations/ordered-blue-scheme|Ordered blue scheme]]
- [[algebraic-geometry-foundations/ordered-blueprint|Ordered blueprint]]
- [[algebraic-geometry-foundations/ordered-blueprint-with-unique-weak-inverses|Ordered blueprint with unique weak inverses]]
- [[algebraic-geometry-foundations/ordered-blueprinted-space|Ordered blueprinted space]]
- [[algebraic-geometry-foundations/parabolic-subgroup|Parabolic subgroup]]
- [[algebraic-geometry-foundations/pasteurization-of-ordered-blueprint|Unique-weak-inverse reflection of an ordered blueprint]]
- [[algebraic-geometry-foundations/pointed-algebraic-curve|Pointed algebraic curve]]
- [[algebraic-geometry-foundations/pre-addition-on-a-monoid|Pre-addition on a monoid]]
- [[algebraic-geometry-foundations/principal-g-bundle-on-scheme|Principal G-bundle on a scheme]]
- [[algebraic-geometry-foundations/projective-geometry|Projective geometry]]
- [[algebraic-geometry-foundations/projective-line|Projective line]]
- [[algebraic-geometry-foundations/projective-line-as-rank-one-flag-variety|The projective line as a rank-one flag variety]]
- [[algebraic-geometry-foundations/projective-morphism|Projective morphism]]
- [[algebraic-geometry-foundations/projective-semilinear-group|Projective semilinear group]]
- [[algebraic-geometry-foundations/projective-transformation|Projective transformation]]
- [[algebraic-geometry-foundations/punctured-algebraic-curve|Punctured algebraic curve]]
- [[algebraic-geometry-foundations/quasi-coherent-sheaf|Quasi-coherent sheaf]]
- [[algebraic-geometry-foundations/reductive-algebraic-group|Reductive algebraic group]]
- [[algebraic-geometry-foundations/ringed-space|Ringed space]]
- [[algebraic-geometry-foundations/semiring-as-a-blueprint|Semiring as a blueprint]]
- [[algebraic-geometry-foundations/semiring-completion-of-a-blueprint|Semiring completion of a blueprint]]
- [[algebraic-geometry-foundations/semirings-and-monoids-as-blueprints|Monoid blueprint to semiring blueprint]]
- [[algebraic-geometry-foundations/sheaf-of-differential-operators|Sheaf of differential operators]]
- [[algebraic-geometry-foundations/sheaf-of-modules|Sheaf of modules]]
- [[algebraic-geometry-foundations/smooth-morphism|Smooth morphism]]
- [[algebraic-geometry-foundations/smooth-projective-curve|Smooth projective curve]]
- [[algebraic-geometry-foundations/spectrum-of-blueprint|Spectrum of a blueprint]]
- [[algebraic-geometry-foundations/tangent-sheaf|Tangent sheaf]]
- [[algebraic-geometry-foundations/weil-restriction|Weil restriction]]

### Formal groups and formal Lie theory (19)

- [[formal-groups|Formal groups and formal group laws]]
- [[formal-groups/additive-and-multiplicative-formal-group-laws|Additive and multiplicative formal group laws]]
- [[formal-groups/affine-formal-groups-and-complete-hopf-algebras|Affine formal groups and complete Hopf algebras]]
- [[formal-groups/complete-filtered-lie-algebra-bch-group|BCH group of a complete filtered Lie algebra]]
- [[formal-groups/coordinate-hopf-algebra|Coordinate Hopf algebra of an affine formal group]]
- [[formal-groups/distribution-algebra|Distribution algebra of a formal group]]
- [[formal-groups/distribution-algebra-and-universal-enveloping-algebra|Distribution algebra and universal enveloping algebra]]
- [[formal-groups/formal-affine-space|Formal affine space]]
- [[formal-groups/formal-completion-at-identity|Formal completion of a group at the identity]]
- [[formal-groups/formal-group|Formal group]]
- [[formal-groups/formal-group-law|Formal group law]]
- [[formal-groups/formal-group-law-morphism|Morphism of formal group laws]]
- [[formal-groups/formal-group-laws-as-coordinates|Formal group laws as coordinates on formal groups]]
- [[formal-groups/formal-group-logarithm|Formal group logarithm]]
- [[formal-groups/height-of-one-dimensional-formal-group-law|Height of a one-dimensional formal group law]]
- [[formal-groups/lie-algebra-formal-group-equivalence|Equivalence between Lie algebras and formal groups]]
- [[formal-groups/one-dimensional-formal-group-law|One-dimensional commutative formal group law]]
- [[formal-groups/positive-characteristic-warning|Failure of tangent classification in positive characteristic]]
- [[formal-groups/tangent-lie-algebra|Tangent Lie algebra of a formal group]]

### Complex analysis and Möbius geometry (53)

- [[complex-analysis|Complex Analysis]]
- [[complex-analysis/analytic-continuation|Analytic continuation]]
- [[complex-analysis/anti-mobius-transformation|Anti-Möbius transformation]]
- [[complex-analysis/argument-principle|Argument principle]]
- [[complex-analysis/automorphisms-of-riemann-sphere|Automorphisms of the Riemann sphere]]
- [[complex-analysis/casorati-weierstrass-theorem|Casorati–Weierstrass theorem]]
- [[complex-analysis/cauchy-integral-formula|Cauchy integral formula]]
- [[complex-analysis/cauchy-integral-theorem|Cauchy integral theorem]]
- [[complex-analysis/cauchy-riemann-criterion|Cauchy–Riemann criterion]]
- [[complex-analysis/cauchy-riemann-equations|Cauchy–Riemann equations]]
- [[complex-analysis/complex-contour-integral|Complex contour integral]]
- [[complex-analysis/complex-derivative|Complex derivative]]
- [[complex-analysis/complex-domain|Complex domain]]
- [[complex-analysis/complex-projective-structure|Complex projective structure]]
- [[complex-analysis/cross-ratio|Cross-ratio]]
- [[complex-analysis/cross-ratio-invariance-under-mobius-transformations|Cross-ratio invariance under Möbius transformations]]
- [[complex-analysis/cross-ratio-preserving-bijections-are-mobius|Cross-ratio-preserving bijections are Möbius]]
- [[complex-analysis/entire-function|Entire function]]
- [[complex-analysis/equal-schwarzians-differ-by-mobius|Equal Schwarzians differ by Möbius postcomposition]]
- [[complex-analysis/fundamental-theorem-of-algebra-complex-analysis|Fundamental theorem of algebra (complex analysis)]]
- [[complex-analysis/generalized-circle|Generalized circle]]
- [[complex-analysis/great-picard-theorem|Great Picard theorem]]
- [[complex-analysis/holomorphic-functions-are-analytic|Holomorphic functions are analytic]]
- [[complex-analysis/holomorphic-germ|Holomorphic germ]]
- [[complex-analysis/identity-theorem|Identity theorem]]
- [[complex-analysis/isolated-singularity-classification|Classification of isolated singularities]]
- [[complex-analysis/laurent-series|Laurent series]]
- [[complex-analysis/liouville-theorem|Liouville's theorem in complex analysis]]
- [[complex-analysis/little-picard-theorem|Little Picard theorem]]
- [[complex-analysis/logarithmic-derivative|Logarithmic derivative]]
- [[complex-analysis/maximum-modulus-principle|Maximum modulus principle]]
- [[complex-analysis/meromorphic-function|Meromorphic function]]
- [[complex-analysis/mobius-characterization-by-schwarzian|Möbius characterization by the Schwarzian]]
- [[complex-analysis/mobius-transformation|Möbius transformation]]
- [[complex-analysis/mobius-transformation-group|Möbius transformation group]]
- [[complex-analysis/monodromy-theorem|Monodromy theorem]]
- [[complex-analysis/montel-theorem|Montel theorem]]
- [[complex-analysis/normal-family|Normal family]]
- [[complex-analysis/open-mapping-theorem|Open mapping theorem for holomorphic functions]]
- [[complex-analysis/order-of-zero-or-pole|Order of a zero or pole]]
- [[complex-analysis/projective-connection|Holomorphic projective connection]]
- [[complex-analysis/projective-connections-form-an-affine-space|Projective connections form an affine space]]
- [[complex-analysis/rational-function|Rational function]]
- [[complex-analysis/residue|Residue]]
- [[complex-analysis/residue-theorem|Residue theorem]]
- [[complex-analysis/riemann-mapping-theorem|Riemann mapping theorem]]
- [[complex-analysis/riemann-sphere|Riemann sphere]]
- [[complex-analysis/rouche-theorem|Rouché's theorem]]
- [[complex-analysis/schwarzian-and-second-order-linear-odes|Schwarzian derivative and second-order linear ODEs]]
- [[complex-analysis/schwarzian-chain-rule|Schwarzian chain rule]]
- [[complex-analysis/schwarzian-derivative|Schwarzian derivative]]
- [[complex-analysis/sharp-three-transitivity-of-mobius-group|Sharp three-transitivity of the Möbius group]]
- [[complex-analysis/winding-number|Winding number]]

### Smooth, complex, symplectic, Kähler, and spin geometry (56)

#### Differential geometry (52)

- [[differential-geometry/almost-complex-map|Almost-complex map]]
- [[differential-geometry/associated-graded-clifford-algebra|Associated graded Clifford algebra]]
- [[differential-geometry/biholomorphism-group|Biholomorphism group]]
- [[differential-geometry/c-infinity-ring|C-infinity ring]]
- [[differential-geometry/category-of-complex-manifolds|Category of complex manifolds]]
- [[differential-geometry/category-of-symplectic-manifolds|Category of symplectic manifolds and symplectic maps]]
- [[differential-geometry/cauchy-hypersurface|Cauchy hypersurface]]
- [[differential-geometry/causal-curve|Causal curve]]
- [[differential-geometry/causal-diamond|Causal diamond]]
- [[differential-geometry/celestial-sphere|Celestial sphere]]
- [[differential-geometry/chirality-operator|Chirality operator]]
- [[differential-geometry/chronological-and-causal-future|Chronological and causal future]]
- [[differential-geometry/clifford-algebra-as-super-enveloping-quotient|Clifford algebra as a super-enveloping quotient]]
- [[differential-geometry/complex-clifford-module-classification|Classification of complex Clifford modules]]
- [[differential-geometry/complex-projective-space|Complex projective space]]
- [[differential-geometry/conformal-map|Conformal map]]
- [[differential-geometry/connection-laplacian|Connection Laplacian]]
- [[differential-geometry/diffeomorphism-groupoid-of-smooth-manifolds|Diffeomorphism groupoid of smooth manifolds]]
- [[differential-geometry/dirac-spinor|Dirac spinor]]
- [[differential-geometry/flux-group|Flux group]]
- [[differential-geometry/flux-homomorphism|Flux homomorphism]]
- [[differential-geometry/global-hyperbolicity-and-cauchy-hypersurfaces|Global hyperbolicity and Cauchy hypersurfaces]]
- [[differential-geometry/globally-hyperbolic-spacetime|Globally hyperbolic spacetime]]
- [[differential-geometry/hamiltonian-isotopy|Hamiltonian isotopy]]
- [[differential-geometry/holomorphic-isometric-immersion|Holomorphic isometric immersion]]
- [[differential-geometry/hyperbolic-three-space|Hyperbolic three-space]]
- [[differential-geometry/hyperkahler-isometric-immersion|Hyperkähler isometric immersion]]
- [[differential-geometry/hyperkahler-isometry|Hyperkähler isometry]]
- [[differential-geometry/laplace-beltrami-operator|Laplace–Beltrami operator]]
- [[differential-geometry/locally-c-infinity-ringed-space|Local C-infinity-ringed space]]
- [[differential-geometry/lorentzian-dirac-operator|Lorentzian Dirac operator]]
- [[differential-geometry/lorentzian-manifold|Lorentzian manifold]]
- [[differential-geometry/lorentzian-spin-structure|Lorentzian spin structure]]
- [[differential-geometry/lorentzian-spinor-bundle|Lorentzian spinor bundle]]
- [[differential-geometry/majorana-spinor|Majorana spinor]]
- [[differential-geometry/majorana-weyl-spinor|Majorana–Weyl spinor]]
- [[differential-geometry/newlander-nirenberg-theorem|Newlander–Nirenberg theorem]]
- [[differential-geometry/nijenhuis-tensor|Nijenhuis tensor of an almost-complex structure]]
- [[differential-geometry/pseudo-riemannian-manifold|Pseudo-Riemannian manifold]]
- [[differential-geometry/real-projective-space|Real projective space]]
- [[differential-geometry/riemannian-isometric-immersion|Riemannian isometric immersion]]
- [[differential-geometry/rotating-hyperkahler-isometry|Rotating hyperkähler isometry]]
- [[differential-geometry/scalar-curvature|Scalar curvature]]
- [[differential-geometry/smooth-maps-from-smooth-function-algebras|Full faithfulness of the smooth-function functor]]
- [[differential-geometry/smooth-splitting-of-globally-hyperbolic-spacetimes|Smooth splitting of globally hyperbolic spacetimes]]
- [[differential-geometry/spinor-module|Spinor module]]
- [[differential-geometry/strong-causality|Strong causality]]
- [[differential-geometry/symplectic-isotopy|Symplectic isotopy]]
- [[differential-geometry/symplectomorphism-group|Symplectomorphism group]]
- [[differential-geometry/time-orientation|Time orientation]]
- [[differential-geometry/triholomorphic-map|Triholomorphic map]]
- [[differential-geometry/weyl-spinor|Weyl spinor]]

#### Bundles, sections, and local systems (4)

- [[fiber-bundles/category-of-vector-bundles-over-a-manifold|Category of vector bundles over a manifold]]
- [[fiber-bundles/local-system|Local system]]
- [[fiber-bundles/sheaf-of-smooth-sections|Sheaf of smooth sections]]
- [[fiber-bundles/vector-bundles-and-locally-free-sheaves|Vector bundles and locally free sheaves]]

### Lie groups and linear models (25)

#### Lie groups and Lie algebras (19)

- [[lie-groups/celestial-sphere-and-mobius-action|Celestial sphere and Möbius action]]
- [[lie-groups/complete-filtered-lie-algebra|Complete filtered Lie algebra]]
- [[lie-groups/complex-lie-group|Complex Lie group]]
- [[lie-groups/complexification-of-a-real-lie-algebra|Complexification of a real Lie algebra]]
- [[lie-groups/hermitian-matrix-model-of-minkowski-space|Hermitian matrix model of Minkowski space]]
- [[lie-groups/indefinite-pin-group|Indefinite pin group]]
- [[lie-groups/indefinite-spin-group|Indefinite spin group]]
- [[lie-groups/lie-functor-commutes-with-realification|Lie functor commutes with realification]]
- [[lie-groups/projective-general-linear-lie-group|Projective general linear Lie group]]
- [[lie-groups/projective-special-linear-lie-group|Projective special linear Lie group]]
- [[lie-groups/proper-orthochronous-lorentz-group|Proper orthochronous Lorentz group]]
- [[lie-groups/psl2-complex|PSL(2,C)]]
- [[lie-groups/psl2c-action-on-hyperbolic-three-space|PSL(2,C) action on hyperbolic three-space]]
- [[lie-groups/psl2c-proper-lorentz-isomorphism|PSL(2,C)–proper Lorentz isomorphism]]
- [[lie-groups/restricted-spin-group|Restricted spin group]]
- [[lie-groups/sl2-complex-as-real-and-complex-lie-group|SL(2,C) as a real and complex Lie group]]
- [[lie-groups/sl2c-spin-cover-of-lorentz-group|SL(2,C) spin cover of the Lorentz group]]
- [[lie-groups/underlying-real-lie-algebra|Underlying real Lie algebra]]
- [[lie-groups/underlying-real-lie-group|Underlying real Lie group]]

#### Linear algebra (6)

- [[linear-algebra/hermitian-matrix|Hermitian matrix]]
- [[linear-algebra/minkowski-vector-space|Minkowski vector space]]
- [[linear-algebra/quadratic-form|Quadratic form]]
- [[linear-algebra/realification-of-a-complex-vector-space|Realification of a complex vector space]]
- [[linear-algebra/semilinear-map|Semilinear map]]
- [[linear-algebra/signature-of-symmetric-bilinear-form|Signature of a symmetric bilinear form]]

### Supergeometry and supersymmetry (27)

- [[supergeometry|Supergeometry]]
- [[supergeometry/batchelor-theorem|Batchelor theorem]]
- [[supergeometry/category-of-super-vector-spaces|Category of super vector spaces]]
- [[supergeometry/functor-of-points-of-supermanifold|Functor of points of a supermanifold]]
- [[supergeometry/koszul-sign-rule|Koszul sign rule]]
- [[supergeometry/lie-superalgebra|Lie superalgebra]]
- [[supergeometry/lie-superalgebra-of-lie-supergroup|Lie superalgebra of a Lie supergroup]]
- [[supergeometry/lie-supergroup|Lie supergroup]]
- [[supergeometry/lie-supergroups-and-super-harish-chandra-pairs|Equivalence of Lie supergroups and super Harish–Chandra pairs]]
- [[supergeometry/parity-shift|Parity shift]]
- [[supergeometry/representation-of-a-lie-superalgebra|Representation of a Lie superalgebra]]
- [[supergeometry/split-supermanifold|Split supermanifold]]
- [[supergeometry/super-harish-chandra-pair|Super Harish–Chandra pair]]
- [[supergeometry/super-internal-hom|Super internal Hom]]
- [[supergeometry/super-minkowski-space|Super-Minkowski space]]
- [[supergeometry/super-pbw-theorem|Super PBW theorem]]
- [[supergeometry/super-vector-space|Super vector space]]
- [[supergeometry/superalgebra|Superalgebra]]
- [[supergeometry/supercommutative-algebra|Supercommutative algebra]]
- [[supergeometry/supercommutator|Supercommutator]]
- [[supergeometry/superdomain|Superdomain]]
- [[supergeometry/supermanifold|Supermanifold]]
- [[supergeometry/supermodule|Supermodule]]
- [[supergeometry/superspace|Superspace]]
- [[supergeometry/supertranslation-distribution|Supertranslation distribution]]
- [[supergeometry/symmetric-algebra-of-a-super-vector-space|Symmetric algebra of a super vector space]]
- [[supergeometry/universal-enveloping-algebra-of-lie-superalgebra|Universal enveloping algebra of a Lie superalgebra]]

### Mathematical physics and hyperbolic equations (21)

- [[mathematical-physics|Mathematical physics]]
- [[mathematical-physics/advanced-and-retarded-green-operators|Advanced and retarded Green operators]]
- [[mathematical-physics/cauchy-problem-for-normally-hyperbolic-operators|Cauchy problem for normally hyperbolic operators]]
- [[mathematical-physics/cauchy-problem-for-the-lorentzian-dirac-operator|Cauchy problem for the Lorentzian Dirac operator]]
- [[mathematical-physics/clifford-slash-notation|Clifford slash notation]]
- [[mathematical-physics/conformal-coupling-of-a-scalar-field|Conformal coupling of a scalar field]]
- [[mathematical-physics/connection-form-of-a-normally-hyperbolic-operator|Connection form of a normally hyperbolic operator]]
- [[mathematical-physics/dalembert-operator|d’Alembert operator]]
- [[mathematical-physics/dirac-equation|Dirac equation]]
- [[mathematical-physics/dirac-klein-gordon-factorization|Dirac–Klein–Gordon factorization]]
- [[mathematical-physics/existence-of-advanced-and-retarded-green-operators|Existence of advanced and retarded Green operators]]
- [[mathematical-physics/gamma-matrices|Gamma matrices]]
- [[mathematical-physics/klein-gordon-equation|Klein–Gordon equation]]
- [[mathematical-physics/klein-gordon-operator|Klein–Gordon operator]]
- [[mathematical-physics/minkowski-dirac-operator|Minkowski Dirac operator]]
- [[mathematical-physics/minkowski-spacetime|Minkowski spacetime]]
- [[mathematical-physics/normally-hyperbolic-operator|Normally hyperbolic operator]]
- [[mathematical-physics/poincare-algebra|Poincaré algebra]]
- [[mathematical-physics/super-poincare-algebra|Super-Poincaré algebra]]
- [[mathematical-physics/supertranslation-algebra|Supertranslation algebra]]
- [[mathematical-physics/wave-equation|Wave equation]]

### Geometric Langlands and its categorical dependencies (33)

- [[langlands|Geometric Langlands]]
- [[langlands/affine-grassmannian|Affine Grassmannian]]
- [[langlands/affine-schubert-variety|Affine Schubert variety]]
- [[langlands/convolution-of-sheaves|Convolution of sheaves on the affine Grassmannian]]
- [[langlands/dominant-coweight|Dominant coweight]]
- [[langlands/g-local-system|G-local system]]
- [[langlands/geometric-langlands-correspondence|Categorical geometric Langlands theorem]]
- [[langlands/geometric-satake-equivalence|Geometric Satake equivalence]]
- [[langlands/global-nilpotent-cone|Global nilpotent cone for local systems]]
- [[langlands/hecke-correspondence|Hecke correspondence]]
- [[langlands/hecke-eigensheaf|Hecke eigensheaf]]
- [[langlands/hecke-functor|Geometric Hecke functor]]
- [[langlands/hecke-modification|Hecke modification]]
- [[langlands/ind-coherent-sheaf|Ind-coherent sheaf]]
- [[langlands/ind-coherent-sheaves-with-nilpotent-singular-support|Ind-coherent sheaves with nilpotent singular support]]
- [[langlands/ind-scheme|Ind-scheme]]
- [[langlands/intersection-cohomology-complex|Intersection-cohomology complex]]
- [[langlands/irregular-singular-connection|Irregular-singular connection]]
- [[langlands/iwahori-level-structure|Iwahori level structure]]
- [[langlands/level-structure-on-g-bundle|Level structure on a G-bundle]]
- [[langlands/loop-group|Loop group]]
- [[langlands/moduli-stack-of-g-local-systems|Moduli stack of G-local systems]]
- [[langlands/parahoric-level-structure|Parahoric level structure]]
- [[langlands/perverse-sheaf|Perverse sheaf]]
- [[langlands/positive-loop-group|Positive loop group]]
- [[langlands/ramification-of-g-local-system|Ramification of a G-local system]]
- [[langlands/ramified-geometric-langlands|Ramified geometric Langlands]]
- [[langlands/regular-singular-connection|Regular-singular connection]]
- [[langlands/riemann-hilbert-correspondence|Riemann–Hilbert correspondence]]
- [[langlands/singular-support-of-coherent-sheaf|Singular support of a coherent sheaf]]
- [[langlands/stokes-data|Stokes data]]
- [[langlands/tame-ramification|Tame ramification]]
- [[langlands/wild-ramification|Wild ramification]]

## Pre-existing knowls substantively refined for integration review (64)

These records existed on `develop` and were expanded, corrected, cross-linked,
or updated as section maps during the July 30 integration. This is the curated
comparison set used by the old/new review page; it excludes corpus-wide
reference-placement cleanup.

### Algebra and algebraic-geometry records (7)

#### Categorical algebra (1)

- [[algebra-category-theory|Category Theory]]

#### Commutative algebra (1)

- [[algebra-commutative|Algebra: Commutative Algebra]]

#### Valuations and fields (1)

- [[algebra-fields-galois|Algebra: Fields and Galois Theory]]

#### Modules and algebra constructions (1)

- [[algebra-modules|Algebra: Module Theory]]

#### Rings, semirings, and power series (2)

- [[algebra-rings|Algebra: Rings]]
- [[algebra-rings/formal-power-series-ring|Formal power series ring]]

#### Algebraic-geometry foundations (1)

- [[algebraic-geometry-foundations/projective-space|Projective space]]

### Differential geometry and bundles (25)

#### Differential geometry (17)

- [[differential-geometry|Differential Geometry]]
- [[differential-geometry/algebra-of-smooth-functions|Algebra of smooth functions]]
- [[differential-geometry/almost-complex-structure|Almost-complex structure]]
- [[differential-geometry/category-of-smooth-manifolds|Category of smooth manifolds]]
- [[differential-geometry/clifford-algebra|Clifford algebra]]
- [[differential-geometry/clifford-module|Clifford module]]
- [[differential-geometry/complex-manifold|Complex manifold]]
- [[differential-geometry/hamiltonian-diffeomorphism|Hamiltonian diffeomorphism]]
- [[differential-geometry/holomorphic-map|Holomorphic map]]
- [[differential-geometry/hyperkahler-manifold|Hyperkähler manifold]]
- [[differential-geometry/integrable-almost-complex-structure|Integrable almost-complex structure]]
- [[differential-geometry/irreducible-holomorphic-symplectic-manifold|Irreducible holomorphic symplectic manifold]]
- [[differential-geometry/kahler-manifold|Kähler manifold]]
- [[differential-geometry/sheaf-of-smooth-functions|Sheaf of smooth functions]]
- [[differential-geometry/spinor-bundle|Spinor bundle]]
- [[differential-geometry/symplectic-manifold|Symplectic manifold]]
- [[differential-geometry/symplectomorphism|Symplectomorphism]]

#### Bundles, sections, and local systems (8)

- [[fiber-bundles|Fiber Bundles]]
- [[fiber-bundles/module-of-smooth-sections|Module of smooth sections]]
- [[fiber-bundles/projective-module-bundle-reconstruction|Reconstructing a vector bundle from a projective module]]
- [[fiber-bundles/section-module-is-finitely-generated-projective|Section module is finitely generated projective]]
- [[fiber-bundles/serre-swan-idempotent-construction|Vector bundle from an idempotent matrix]]
- [[fiber-bundles/serre-swan-theorem|Serre–Swan theorem]]
- [[fiber-bundles/universal-principal-bundle-egbg|Universal principal bundle EG→BG]]
- [[fiber-bundles/vector-bundle-morphism|Vector bundle morphism]]

### Analysis, noncommutative geometry, and operator algebras (20)

#### Functional analysis (3)

- [[functional-analysis/decomposable-operator|Decomposable operator]]
- [[functional-analysis/fourier-transform-schwartz-space|Fourier transform on Schwartz space]]
- [[functional-analysis/fourier-transform-tempered-distributions|Fourier transform of tempered distributions]]

#### Harmonic analysis (3)

- [[harmonic-analysis/quasi-invariant-measure|Quasi-invariant measure under a locally compact group action]]
- [[harmonic-analysis/system-of-imprimitivity|System of imprimitivity]]
- [[harmonic-analysis/weil-integration-formula|Weil integration formula]]

#### Noncommutative geometry (2)

- [[noncommutative-geometry|Noncommutative Geometry]]
- [[noncommutative-geometry/dirac-operator|Dirac operator]]

#### Operator algebras (11)

- [[operator-algebras/compact-operator-cstar-algebra|C*-algebra of compact operators]]
- [[operator-algebras/continuous-core-von-neumann-algebra|Continuous core of a von Neumann algebra]]
- [[operator-algebras/factorial-representation|Factorial representation]]
- [[operator-algebras/faithful-normal-semifinite-trace|Faithful normal semifinite trace]]
- [[operator-algebras/modular-automorphism-group|Modular automorphism group]]
- [[operator-algebras/normal-completely-positive-map|Normal completely positive map]]
- [[operator-algebras/nuclear-cstar-algebra|Nuclear C*-algebra]]
- [[operator-algebras/nuclear-implies-exact|Nuclear C*-algebras are exact]]
- [[operator-algebras/quasi-equivalent-representations|Quasi-equivalent representations]]
- [[operator-algebras/tomita-takesaki-theorem|Tomita–Takesaki theorem]]
- [[operator-algebras/type-i-infinity-factor|Type I∞ factor]]

#### Real analysis (1)

- [[real-analysis/power-series|Power series]]

### Langlands, Lie theory, and linear algebra (11)

#### Langlands foundations (2)

- [[langlands-letter/knowls/langlands-dual-group|Langlands Dual Group]]
- [[langlands-letter/knowls/root-vs-weight-lattice-isogeny|Root Lattice, Weight Lattice, and Isogeny Forms]]

#### Lie groups and Lie algebras (8)

- [[lie-groups|Lie Groups and Lie Algebras]]
- [[lie-groups/baker-campbell-hausdorff-formula|Baker–Campbell–Hausdorff formula]]
- [[lie-groups/lie-algebra|Lie algebra]]
- [[lie-groups/lorentz-group|Lorentz group]]
- [[lie-groups/nelson-laplacian|Nelson Laplacian]]
- [[lie-groups/special-linear-group|Special linear group]]
- [[lie-groups/special-linear-lie-algebra|Special linear Lie algebra]]
- [[lie-groups/spin-group|Spin group]]

#### Linear algebra (1)

- [[linear-algebra|Linear Algebra]]

### Earlier review documentation (1)

- [[knowlification/july-26-additions|July 2026 operator algebra and geometry additions]]
