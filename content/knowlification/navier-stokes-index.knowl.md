+++
id = "knowlification/navier-stokes-index"
title = "Navier–Stokes: new knowl collection"
kind = "index"
summary = "The 433 knowls from batches B00–B14 plus one document-integration addition, grouped by batch and subject."
aliases = ["Navier prerequisite collection", "Navier–Stokes batch index"]
domains = ["knowlification"]
section_mode = "progressive"
prerequisites = []
dependency_heuristic = "navier-semantic-review-v1"
dependency_review_count = 1
+++

This collection contains **434 new knowls: 433 in 15 batches and one document-integration addition**, from shared foundations to fluid equations, oscillatory constructions, and energy methods. Open a batch below, then unfold any entry and its prerequisites. Each entry appears in its original batch and is grouped by mathematical subject.

The batch order is a suggested route through the material. The prerequisite links inside each knowl give the actual dependencies; earlier batch numbers alone do not define that graph.

## B00 · Shared foundations and indexing notation

14 new knowls.

### Foundations

- [[shared-foundations/binomial-coefficient|Binomial coefficient]]
- [[shared-foundations/ceiling-function|Ceiling function]]
- [[shared-foundations/finite-product|Finite product]]
- [[shared-foundations/finite-set|Finite set]]
- [[shared-foundations/finite-sum|Finite sum]]
- [[shared-foundations/first-order-logic|First-order logic and inference rules]]
- [[shared-foundations/floor-function|Floor function]]
- [[shared-foundations/mathematical-axiom|Mathematical axiom]]
- [[shared-foundations/mathematical-proof|Mathematical proof]]
- [[shared-foundations/mathematical-proposition|Mathematical proposition]]
- [[shared-foundations/mathematical-theorem|Mathematical theorem]]
- [[shared-foundations/nearest-integer|Nearest integer]]
- [[shared-foundations/rising-factorial|Rising factorial]]
- [[shared-foundations/telescoping-sum|Telescoping sum]]

## B01 · Linear algebra, elementary functions and Euclidean vector calculus

37 new knowls.

### Linear algebra

- [[linear-algebra/block-matrix|Block matrix]]
- [[linear-algebra/cross-product|Cross product in three-dimensional Euclidean space]]
- [[linear-algebra/dual-basis|Dual basis]]
- [[linear-algebra/elementary-row-operation|Elementary row operation]]
- [[linear-algebra/hadamard-product|Hadamard product]]
- [[linear-algebra/matrix-inverse|Inverse matrix]]
- [[linear-algebra/left-inverse|Left inverse of a linear map]]
- [[linear-algebra/moving-frame|Moving frame for a family of subspaces]]
- [[linear-algebra/reflection|Orthogonal reflection across a hyperplane]]
- [[linear-algebra/outer-product|Outer product]]
- [[linear-algebra/quadratic-map|Quadratic map over the real numbers]]
- [[linear-algebra/right-inverse|Right inverse of a linear map]]
- [[linear-algebra/unit-vector|Unit vector]]

### Real and multivariable analysis

- [[real-analysis/cylindrical-axis-regularity|A sufficient criterion for smoothness at a cylindrical axis]]
- [[real-analysis/cartesian-coordinates|Cartesian coordinates]]
- [[real-analysis/completing-the-square|Completing the square]]
- [[real-analysis/cosine-function|Cosine function]]
- [[real-analysis/curl|Curl of a three-dimensional vector field]]
- [[real-analysis/cylindrical-coordinates|Cylindrical coordinates and their orthonormal frame]]
- [[real-analysis/divergence|Divergence of a Euclidean vector field]]
- [[real-analysis/divergence-free-field|Divergence-free field]]
- [[real-analysis/laplacian|Euclidean Laplacian]]
- [[real-analysis/even-and-odd-functions|Even and odd parity of a function]]
- [[real-analysis/exponential-function|Exponential function]]
- [[real-analysis/logarithmic-radial-derivative|Logarithmic radial derivative]]
- [[real-analysis/tensor-field|Matrix-valued tensor field in Euclidean coordinates]]
- [[real-analysis/natural-logarithm|Natural logarithm]]
- [[real-analysis/nonnegative-square-root|Nonnegative square root]]
- [[real-analysis/product-rule|Product rule for derivatives]]
- [[real-analysis/quotient-rule|Quotient rule]]
- [[real-analysis/real-power|Real power of a positive number]]
- [[real-analysis/divergence-of-tensor|Row divergence of a matrix field]]
- [[real-analysis/sine-function|Sine function]]
- [[real-analysis/stokes-streamfunction|Stokes streamfunction for an axisymmetric meridional field]]
- [[real-analysis/euclidean-vector-field|Vector field on a Euclidean open set]]
- [[real-analysis/vector-potential|Vector potential for a divergence-free field]]

### Topology

- [[topology/annulus|Euclidean annulus]]

## B02 · Measure, function spaces and core inequalities

22 new knowls.

### Function spaces and operators

- [[functional-analysis/absolutely-convergent-banach-series|Absolutely convergent series in a Banach space]]
- [[functional-analysis/bounded-bilinear-map|Bounded bilinear map between normed spaces]]
- [[functional-analysis/continuous-banach-valued-functions|Continuous Banach-valued functions on a compact space]]
- [[functional-analysis/integral-operator|Integral operator with a measurable kernel]]

### Measure and integration

- [[measure-theory/bochner-integral|Bochner integral]]
- [[measure-theory/complete-measure|Complete measure]]
- [[measure-theory/completeness-of-lp|Completeness of Lebesgue spaces]]
- [[measure-theory/measure-completion|Completion of a measure space]]
- [[measure-theory/differentiation-under-integral|Differentiation under an integral sign]]
- [[measure-theory/hausdorff-dimension|Hausdorff dimension]]
- [[measure-theory/hausdorff-measure|Hausdorff measure in a metric space]]
- [[measure-theory/integrable-majorant|Integrable majorant for a family]]
- [[measure-theory/lp-interpolation-inequality|Interpolation between two Lebesgue norms]]
- [[measure-theory/locally-integrable-function|Locally integrable function]]
- [[measure-theory/mixed-lebesgue-norm|Mixed Lebesgue norm]]
- [[measure-theory/parabolic-hausdorff-measure|Parabolic Hausdorff measure]]
- [[measure-theory/product-sigma-algebra|Product sigma-algebra]]
- [[measure-theory/generated-sigma-algebra|Sigma-algebra generated by a family]]
- [[measure-theory/sigma-finite-measure|Sigma-finite measure]]
- [[measure-theory/strongly-measurable-function|Strongly measurable Banach-valued function]]

### Real and multivariable analysis

- [[real-analysis/nonnegative-extended-series|Sum of nonnegative extended real terms]]

### Topology

- [[topology/parabolic-metric|Parabolic metric on space-time]]

## B03 · ODE existence, stability and analytic dependence

37 new knowls.

### Complex analysis

- [[complex-analysis/cauchy-derivative-estimate|Cauchy estimate for holomorphic derivatives]]
- [[complex-analysis/common-holomorphic-neighborhood|Common holomorphic neighborhood for a parameter family]]
- [[complex-analysis/holomorphic-parameter-integral|Holomorphic dependence of a parameter integral]]
- [[complex-analysis/holomorphic-logarithm|Holomorphic logarithm of a nonvanishing function]]
- [[complex-analysis/locally-uniform-limit-theorem|Locally uniform limit theorem for holomorphic functions]]
- [[complex-analysis/nonvanishing-neighborhood|Nonvanishing on a neighborhood of a compact set]]
- [[complex-analysis/power-of-holomorphic-function|Power of a holomorphic function using a chosen logarithm]]

### Function spaces and operators

- [[functional-analysis/mixed-derivative-radial-estimate|A mixed-derivative estimate after radial inversion]]
- [[functional-analysis/contraction-on-a-closed-ball|Contraction criterion on a closed norm ball]]
- [[functional-analysis/analytic-coefficient-realization|Holomorphic realization of weighted analytic coefficients]]
- [[functional-analysis/factorial-operator-inverse|Operator inversion from summable bounds on powers]]
- [[functional-analysis/analytic-coefficient-algebra|Product estimate in a two-index analytic coefficient space]]
- [[functional-analysis/analytic-coefficient-radial-estimates|Radial operator bounds in an analytic coefficient norm]]
- [[functional-analysis/two-index-analytic-coefficient-space|Two-index analytic coefficient space]]

### Linear algebra

- [[linear-algebra/matrix-exponential|Matrix exponential]]

### Measure and integration

- [[measure-theory/ordered-integration-simplex|Ordered integration simplex]]

### Ordinary differential equations

- [[differential-equations/continuation-criterion|Continuation criterion for a finite-dimensional ODE]]
- [[differential-equations/continuous-dependence-estimate|Continuous-dependence estimate for ODEs]]
- [[differential-equations|Differential equations]]
- [[differential-equations/duhamel-formula|Duhamel formula for a linear ODE]]
- [[differential-equations/fundamental-matrix|Fundamental matrix and principal propagator]]
- [[differential-equations/gronwall-inequality|Gronwall inequality]]
- [[differential-equations/holomorphic-parameter-dependence|Holomorphic dependence of local ODE solutions]]
- [[differential-equations/initial-value-problem|Initial-value problem for an ODE]]
- [[differential-equations/integrating-factor|Integrating factor for a scalar linear ODE]]
- [[differential-equations/linear-ode|Linear ordinary differential system]]
- [[differential-equations/ordinary-differential-equation|Ordinary differential equation]]
- [[differential-equations/peano-baker-series|Peano–Baker series]]
- [[differential-equations/picard-iteration|Picard iteration]]
- [[differential-equations/picard-lindelof-theorem|Picard–Lindelöf local existence and uniqueness theorem]]
- [[differential-equations/radial-regular-inverse|Regular inverse of Y d2/dY2 plus nu d/dY]]
- [[differential-equations/regular-singular-point|Regular singular point of a second-order linear ODE]]
- [[differential-equations/comparison-barrier|Scalar comparison barriers for an ODE]]
- [[differential-equations/smooth-parameter-dependence|Smooth dependence of ODE solutions on parameters]]

### Real and multivariable analysis

- [[real-analysis/radial-averaging-operator|Radial averaging operator from the origin]]
- [[real-analysis/real-analytic-function|Real-analytic function]]

### Topology

- [[topology/locally-lipschitz-map|Locally Lipschitz map]]

## B04 · Smooth localization and extension

21 new knowls.

### Real and multivariable analysis

- [[real-analysis/borel-jet-extension|Borel extension of a prescribed smooth jet]]
- [[real-analysis/cartesian-jet|Cartesian jet of a function]]
- [[real-analysis/compact-derivative-seminorm|Compact derivative seminorm]]
- [[real-analysis/compactly-supported-function|Compactly supported function]]
- [[real-analysis/scaled-cutoff-estimate|Derivative estimates for scaled cutoffs]]
- [[real-analysis/flat-exponential|Flat exponential]]
- [[real-analysis/flat-factor-product|Flat factors absorb polynomial derivative growth]]
- [[real-analysis/flat-function|Flat function along a set]]
- [[real-analysis/hadamard-division-lemma|Hadamard division lemma in one normal variable]]
- [[real-analysis/solenoidal-localization|Localization through a vector potential]]
- [[real-analysis/multi-index-leibniz-rule|Multi-index Leibniz rule]]
- [[real-analysis/normalized-flat-primitive|Normalized primitive of an exponential flat factor]]
- [[real-analysis/cutoff-function|Smooth cutoff function]]
- [[real-analysis/smooth-extension|Smooth extension of a function]]
- [[real-analysis/smooth-zero-extension|Smooth zero extension across a flat boundary]]
- [[real-analysis/squared-partition-of-unity|Squared partition of unity]]

### Topology

- [[topology/compact-exhaustion|Compact exhaustion of an open Euclidean set]]
- [[topology/compact-support-margin|Positive margin around a compact subset of an open set]]

### Analysis

- [[analysis/locally-finite-smooth-sum|Locally finite sum of smooth functions]]
- [[analysis/bounded-overlap-sum-estimate|Lp estimate for sums with bounded overlap]]
- [[analysis/smooth-series-convergence|Smooth convergence of a series from derivative bounds]]

## B05 · Incompressible fluid equations

50 new knowls.

### Fluid dynamics

- [[fluid-dynamics/transport-time|Advective transport time scale]]
- [[fluid-dynamics/affine-flow|Affine velocity field]]
- [[fluid-dynamics/angular-velocity|Angular velocity about an axis]]
- [[fluid-dynamics/angular-velocity-equation|Angular-velocity equation in axisymmetric flow]]
- [[fluid-dynamics/axial-velocity|Axial velocity]]
- [[fluid-dynamics/axisymmetric-navier-stokes|Axisymmetric Navier–Stokes component equations]]
- [[fluid-dynamics/axisymmetric-field|Axisymmetric vector field]]
- [[fluid-dynamics/centripetal-acceleration|Centripetal acceleration for circular motion]]
- [[fluid-dynamics/conservative-momentum-equation|Conservative incompressible momentum equation]]
- [[fluid-dynamics/external-force|External force in a normalized fluid equation]]
- [[fluid-dynamics|Fluid dynamics]]
- [[fluid-dynamics/velocity-field|Fluid velocity field]]
- [[fluid-dynamics/euler-equations|Incompressible Euler equations]]
- [[fluid-dynamics/navier-stokes-equations|Incompressible Navier–Stokes equations]]
- [[fluid-dynamics/incompressible-flow|Incompressible velocity field]]
- [[fluid-dynamics/kinetic-energy|Kinetic energy of an incompressible velocity field]]
- [[fluid-dynamics/material-derivative|Material derivative]]
- [[fluid-dynamics/meridional-velocity|Meridional velocity]]
- [[fluid-dynamics/momentum-flux|Momentum flux tensor in an incompressible fluid]]
- [[fluid-dynamics/shear-flow|Parallel shear flow]]
- [[fluid-dynamics/particle-trajectory|Particle trajectory of a velocity field]]
- [[fluid-dynamics/pressure-field|Pressure in incompressible flow]]
- [[fluid-dynamics/quadratic-advection|Quadratic advection of a velocity field]]
- [[fluid-dynamics/radial-velocity|Radial velocity in cylindrical coordinates]]
- [[fluid-dynamics/rate-of-strain|Rate-of-strain tensor]]
- [[fluid-dynamics/reynolds-number|Reynolds number]]
- [[fluid-dynamics/rotating-flow|Rotating flow about a fixed axis]]
- [[fluid-dynamics/specific-angular-momentum|Specific angular momentum about an axis]]
- [[fluid-dynamics/swirl|Swirl component of a cylindrical velocity]]
- [[fluid-dynamics/transport-operator|Transport operator along a vector field]]
- [[fluid-dynamics/viscosity|Viscosity in the incompressible Newtonian model]]
- [[fluid-dynamics/diffusion-time|Viscous diffusion time scale]]
- [[fluid-dynamics/vortex-line|Vortex line]]
- [[fluid-dynamics/vorticity-equation|Vorticity equation for incompressible flow]]
- [[fluid-dynamics/vorticity|Vorticity of a velocity field]]

### Linear algebra

- [[linear-algebra/symmetric-matrix|Symmetric matrix]]
- [[linear-algebra/matrix-transpose|Transpose of a matrix]]

### Partial differential equations

- [[partial-differential-equations/cauchy-problem|Cauchy problem for an evolution PDE]]
- [[partial-differential-equations/classical-solution|Classical solution of a PDE]]
- [[partial-differential-equations/conservation-law|Conservation law in divergence form]]
- [[partial-differential-equations/finite-time-blowup|Finite-time blowup in a specified norm]]
- [[partial-differential-equations/initial-datum|Initial datum for an evolution equation]]
- [[partial-differential-equations/linear-differential-operator|Linear partial differential operator]]
- [[partial-differential-equations/local-well-posedness|Local well-posedness of an evolution problem]]
- [[partial-differential-equations/maximal-existence-time|Maximal existence time in a solution class]]
- [[partial-differential-equations/partial-differential-equation|Partial differential equation]]
- [[partial-differential-equations|Partial differential equations]]
- [[partial-differential-equations/principal-part|Principal part of a differential operator]]
- [[partial-differential-equations/transport-equation|Transport equation]]

### Real and multivariable analysis

- [[real-analysis/limit-superior-at-an-endpoint|Limit superior of a function at an endpoint]]

## B06 · Perturbation theory and anisotropic asymptotics

38 new knowls.

### Asymptotic estimates

- [[asymptotics/logarithmic-loss-absorption|Absorbing logarithmic losses into a power]]
- [[asymptotics/asymptotic-equivalence|Asymptotic equivalence]]
- [[asymptotics/asymptotic-expansion|Asymptotic expansion]]
- [[asymptotics/asymptotic-scale|Asymptotic scale]]
- [[asymptotics/big-o|Big-O notation]]
- [[asymptotics/comparable-functions|Comparable positive functions]]
- [[asymptotics/concentration-scale|Concentration scale]]
- [[asymptotics/finite-truncation|Finite truncation of an expansion]]
- [[asymptotics/formal-expansion|Formal expansion]]
- [[asymptotics/parameter-choice-hierarchy|Hierarchy of parameter choices]]
- [[asymptotics/leading-order|Leading order]]
- [[asymptotics/leading-term|Leading term of an asymptotic expansion]]
- [[asymptotics/little-o|Little-o notation]]
- [[asymptotics/perturbation|Perturbation]]
- [[asymptotics/power-law|Power law]]
- [[asymptotics/quadratic-self-interaction|Quadratic self-interaction]]
- [[asymptotics/remainder-estimate|Remainder estimate]]
- [[asymptotics/uniform-parameter-estimate|Uniform parameter estimate]]

### Fluid dynamics

- [[fluid-dynamics/linearized-navier-stokes|Linearized Navier–Stokes operator]]
- [[fluid-dynamics/momentum-residual|Navier–Stokes momentum residual]]
- [[fluid-dynamics/viscosity-rescaling|Rescaling unit viscosity to positive viscosity]]
- [[fluid-dynamics/navier-stokes-scaling|Scaling symmetry of Navier–Stokes]]

### Function spaces and operators

- [[functional-analysis/derivative-loss|Derivative loss in an estimate]]

### Partial differential equations

- [[partial-differential-equations/differential-polynomial|Differential polynomial]]
- [[partial-differential-equations/self-similar-profile|Self-similar profile]]
- [[partial-differential-equations/similarity-variables|Similarity variables]]

### Real and multivariable analysis

- [[real-analysis/anisotropic-dilation|Anisotropic dilation]]
- [[real-analysis/implicit-similarity-chain-rule|Chain rule in implicit anisotropic coordinates]]
- [[real-analysis/envelope-controlled-amplitude|Envelope-controlled amplitude]]
- [[real-analysis/flat-logarithmic-interval-weight|Flat logarithmic interval weight]]
- [[real-analysis/implicit-anisotropic-coordinates|Implicit anisotropic similarity coordinates]]
- [[real-analysis/linearization|Linearization of a nonlinear map]]
- [[real-analysis/normalized-dyadic-chart|Normalized dyadic chart]]
- [[real-analysis/concentrating-profile-norms|Norms of an anisotropically concentrating profile]]
- [[real-analysis/weighted-coefficient-algebra|Product bounds for weighted coefficients and amplitudes]]
- [[real-analysis/weighted-radial-moment-scaling|Scaling of weighted radial moments]]
- [[real-analysis/dyadic-scale-partition|Smooth dyadic partition of a positive scale]]
- [[real-analysis/weighted-coefficient-class|Weighted coefficient class]]

## B07 · Fourier analysis, oscillatory waves and instability

35 new knowls.

### Fluid dynamics

- [[fluid-dynamics/angular-average|Angular average in cylindrical components]]
- [[fluid-dynamics/curl-completion-of-an-oscillatory-wave|Curl completion of a transverse oscillatory wave]]
- [[fluid-dynamics/phase-transport-defect|Phase transport defect]]
- [[fluid-dynamics/shearing-wave|Shearing wave on an affine flow]]
- [[fluid-dynamics/viscous-damping-of-a-mode|Viscous damping of a Fourier mode]]

### Fourier and harmonic analysis

- [[harmonic-analysis/carrier-frequency|Carrier frequency parameter]]
- [[harmonic-analysis/smooth-fourier-coefficient-decay|Decay of smooth periodic Fourier coefficients]]
- [[harmonic-analysis/fourier-character|Fourier character on the standard torus]]
- [[harmonic-analysis/fourier-harmonic|Fourier harmonic]]
- [[harmonic-analysis/gaussian-pulse-envelope|Gaussian bound from a decreasing net growth rate]]
- [[harmonic-analysis/wavevector|Local wavevector]]
- [[harmonic-analysis/fourier-product-mean|Mean of a product of periodic Fourier sums]]
- [[harmonic-analysis/fourier-orthogonality|Orthogonality of periodic Fourier characters]]
- [[harmonic-analysis/oscillatory-modulation|Oscillatory modulation]]
- [[harmonic-analysis/oscillatory-phase|Oscillatory phase]]
- [[harmonic-analysis/oscillatory-pulse|Oscillatory pulse]]
- [[harmonic-analysis/periodic-antiderivative|Periodic antiderivative]]
- [[harmonic-analysis/fourier-coefficient|Periodic Fourier coefficient]]
- [[harmonic-analysis/periodic-fourier-multiplier|Periodic Fourier multiplier]]
- [[harmonic-analysis/fourier-series|Periodic Fourier series]]
- [[harmonic-analysis/pulse-envelope|Pulse envelope]]
- [[harmonic-analysis/smooth-fourier-reconstruction|Smooth periodic Fourier reconstruction]]
- [[harmonic-analysis/transverse-polarization|Transverse polarization of a vector wave]]
- [[harmonic-analysis/wave-amplitude|Wave amplitude]]
- [[harmonic-analysis/wavelength|Wavelength]]
- [[harmonic-analysis/wavenumber|Wavenumber]]
- [[harmonic-analysis/zero-fourier-mode|Zero Fourier mode]]

### Measure and integration

- [[measure-theory/zero-mean-projection|Projection onto zero-mean functions]]
- [[measure-theory/zero-mean-function|Zero-mean function]]

### Ordinary differential equations

- [[differential-equations/envelope-duhamel-estimate|Duhamel estimate with a common envelope]]
- [[differential-equations/scalar-damping-factorization|Factoring scalar damping from a matrix evolution]]
- [[differential-equations/linear-instability|Instability of a linear equilibrium]]
- [[differential-equations/moving-normal-constraint|Linear evolution under a moving normal constraint]]

### Real and multivariable analysis

- [[real-analysis/auxiliary-variable-evaluation|Evaluation along an auxiliary map]]
- [[real-analysis/periodic-function|Periodic function]]

## B08 · Tori, coverings, small divisors and support combinatorics

35 new knowls.

### Asymptotic estimates

- [[asymptotics/logarithmic-iteration-selection|Logarithmic selection of an iteration level]]

### Foundations

- [[shared-foundations/descent-through-surjection|Descent of a function through a surjection]]
- [[shared-foundations/strict-partial-order|Strict partial order]]

### Fourier and harmonic analysis

- [[harmonic-analysis/diophantine-direction|Diophantine direction]]
- [[harmonic-analysis/diophantine-directional-inverse|Directional inverse with a Diophantine bound]]
- [[harmonic-analysis/torus-cover-fourier-descent|Fourier criterion for descent under a torus covering]]
- [[harmonic-analysis/near-resonant-fourier-mode|Nearly resonant Fourier mode]]
- [[harmonic-analysis/normalized-torus-average|Normalized torus average]]
- [[harmonic-analysis/resonant-fourier-mode|Resonant mode of a directional derivative]]
- [[harmonic-analysis/small-divisor|Small divisor]]
- [[harmonic-analysis/torus-cover-preserves-average|Torus coverings preserve normalized averages]]

### Linear algebra

- [[linear-algebra/euclidean-lattice|Full-rank Euclidean lattice]]
- [[linear-algebra/lattice-fundamental-domain|Fundamental domain of a Euclidean lattice]]
- [[linear-algebra/integer-lattice-index|Index of an integer sublattice]]

### Real and multivariable analysis

- [[real-analysis/chain-rule-for-iterated-torus-variables|Chain rule for iterated torus variables]]
- [[real-analysis/irrational-number|Irrational number]]
- [[real-analysis/quadratic-algebraic-conjugate|Quadratic algebraic conjugate]]
- [[real-analysis/quadratic-irrational|Quadratic irrational]]
- [[real-analysis/quadratic-irrational-lower-bound|Quadratic irrational lower bound]]

### Topology

- [[topology/common-torus-for-iterated-covers|Common torus for iterated integer coverings]]
- [[topology/deck-transformation|Deck transformation]]
- [[topology/flat-torus|Flat torus]]
- [[topology/integer-matrix-torus-cover|Integer-matrix torus covering]]
- [[topology/local-lifts-of-torus-cells|Local lifts of cells under a torus covering]]
- [[topology/separated-centers-under-torus-covers|Separated centers under finitely many torus covers]]

### Analysis

- [[analysis/separated-support-products|Vanishing products from separated auxiliary supports]]

### Discrete structures

- [[discrete-structures/countable-graph|Countable graph]]
- [[discrete-structures/directed-acyclic-graph|Directed acyclic graph]]
- [[discrete-structures/directed-cycle|Directed cycle]]
- [[discrete-structures/directed-graph|Directed graph]]
- [[discrete-structures/directed-path|Directed path]]
- [[discrete-structures/greedy-coloring-bounded-degree|Greedy coloring of a countable bounded-degree graph]]
- [[discrete-structures/proper-graph-coloring|Proper graph coloring]]
- [[discrete-structures/support-intersection-graph|Support intersection graph]]
- [[discrete-structures/vertex-degree|Vertex degree]]

## B09 · Convex geometry, covariance and constrained moments

29 new knowls.

### Convex geometry

- [[convex-analysis/conical-combination|Conical combination]]
- [[convex-analysis/convex-cone|Convex cone]]
- [[convex-analysis/planar-quadratic-cone|Planar cone defined by a quadratic inequality]]
- [[convex-analysis/positive-span|Positive span]]
- [[convex-analysis/two-generator-cone-test|Two-generator cone test]]
- [[convex-analysis/uniform-cone-margin|Uniform directional margin inside a cone]]

### Fluid dynamics

- [[fluid-dynamics/radial-tangential-covariance|Radial-tangential velocity covariance]]

### Function spaces and operators

- [[functional-analysis/small-solution-of-a-quadratic-equation|Small solution of an invertible linear equation with quadratic error]]

### Linear algebra

- [[linear-algebra/linearized-squared-amplitude-inverse|Linearized inverse for squared amplitudes]]
- [[linear-algebra/moment-pairing-matrix|Moment pairing matrix]]
- [[linear-algebra/positive-quadratic-realization|Positive realization by squared amplitudes]]
- [[linear-algebra/vandermonde-matrix|Vandermonde matrix]]

### Measure and integration

- [[measure-theory/averaged-second-moment-matrix|Averaged second-moment matrix]]

### Partial differential equations

- [[partial-differential-equations/solvability-compatibility-condition|Solvability compatibility condition]]

### Probability and moments

- [[probability/covariance-matrix|Covariance matrix]]
- [[probability/derivatives-of-log-mgf|Derivatives of the log moment generating function]]
- [[probability/exponential-tilt|Exponential tilt]]
- [[probability/variance-of-normalized-exponential-weight|Variance of a normalized exponential weight]]

### Real and multivariable analysis

- [[real-analysis/reparametrization-by-positive-circle-density|Circle reparametrization by a positive density]]
- [[real-analysis/supported-weighted-radial-primitive|Compactly supported weighted radial primitive]]
- [[real-analysis/cumulative-weighted-integral|Cumulative weighted integral]]
- [[real-analysis/coalescing-moment-exponents|Inverse loss when two moment exponents coalesce]]
- [[real-analysis/invertible-ordered-bump-moments|Invertibility of moments on ordered bump supports]]
- [[real-analysis/moment-matching|Matching prescribed integral moments]]
- [[real-analysis/positive-powers-of-flat-weighted-profiles|Positive powers of flat weighted profiles]]
- [[real-analysis/preservation-of-exterior-primitives|Preservation of exterior primitives by moment matching]]
- [[real-analysis/weighted-moment-projection|Projection removing one weighted moment]]
- [[real-analysis/weighted-radial-moment|Weighted radial moment]]
- [[real-analysis/zero-count-for-distinct-powers|Zero count for distinct real powers]]

## B10 · Heat equations and special-function integral profiles

20 new knowls.

### Fluid dynamics

- [[fluid-dynamics/swirl-diffusion-operator|Cylindrical swirl diffusion operator]]
- [[fluid-dynamics/integrability-of-swirl-heat-tail|Integrability of the subtracted swirl heat tail]]
- [[fluid-dynamics/integral-profile-for-swirl-heat-flow|Integral profile for radial swirl heat flow]]
- [[fluid-dynamics/pure-swirl-heat-solution|Pure swirl solution from a radial heat flow]]

### Measure and integration

- [[measure-theory/reference-subtracted-integral|Reference-subtracted integral]]

### Ordinary differential equations

- [[differential-equations/equation-for-gamma-average|Differential equation for the Gamma average]]

### Partial differential equations

- [[partial-differential-equations/directional-diffusion|Diffusion in a fixed direction]]
- [[partial-differential-equations/duhamel-formula-for-heat|Duhamel formula for the heat equation]]
- [[partial-differential-equations/euclidean-heat-kernel|Euclidean heat kernel]]
- [[partial-differential-equations/heat-equation|Heat equation]]
- [[partial-differential-equations/heat-semigroup|Heat semigroup]]
- [[partial-differential-equations/heat-smoothing-estimate|Heat smoothing estimate]]
- [[partial-differential-equations/heat-kernel-solution|Solution of the heat equation by Gaussian convolution]]

### Real and multivariable analysis

- [[real-analysis/gamma-average-profile|Gamma average of an algebraic factor]]
- [[real-analysis/gamma-function|Gamma function on positive real arguments]]
- [[real-analysis/gaussian-integral|Gaussian integral]]
- [[real-analysis/radial-laplacian|Radial Laplacian]]
- [[real-analysis/smooth-endpoint-of-gamma-average|Smooth nonanalytic endpoint of a Gamma average]]
- [[real-analysis/pi|The constant pi]]
- [[real-analysis/tricomi-u-positive-arguments|Tricomi U for positive real arguments]]

## B11 · Distributions, Sobolev spaces, singular integrals and pressure

27 new knowls.

### Fluid dynamics

- [[fluid-dynamics/canonical-pressure-from-integrable-stress|Canonical pressure from an integrable tensor]]

### Fourier and harmonic analysis

- [[harmonic-analysis/calderon-zygmund-kernel|Calderón–Zygmund kernel]]
- [[harmonic-analysis/calderon-zygmund-operator|Calderón–Zygmund operator]]
- [[harmonic-analysis/euclidean-l2-fourier-multiplier|Euclidean L2 Fourier multiplier]]
- [[harmonic-analysis/multiplication-commutator-kernel|Kernel cancellation in a multiplication commutator]]
- [[harmonic-analysis/double-riesz-kernel-three-dimensions|Kernel of a double Riesz transform in three dimensions]]
- [[harmonic-analysis/calderon-zygmund-lp-bound|Lp boundedness of Calderón–Zygmund operators]]
- [[harmonic-analysis/principal-value-singular-integral|Principal-value singular integral]]
- [[harmonic-analysis/riesz-transform|Riesz transform]]
- [[harmonic-analysis/scaled-cutoff-commutator-estimate|Scaled cutoff commutator estimate]]

### Function spaces and operators

- [[functional-analysis/sobolev-derivative-bound|Derivative bound between Fourier Sobolev spaces]]
- [[functional-analysis/dirac-delta-distribution|Dirac delta distribution]]
- [[functional-analysis/fourier-sobolev-space|Fourier Sobolev space on Euclidean space]]
- [[functional-analysis/l1-negative-sobolev-bound|Integrable functions in negative Sobolev spaces]]
- [[functional-analysis/local-sobolev-space|Local Sobolev space]]
- [[functional-analysis/multiplication-operator|Multiplication operator]]
- [[functional-analysis/smooth-density-in-euclidean-sobolev-space|Smooth compact-support approximation in Euclidean Sobolev space]]
- [[functional-analysis/sobolev-embedding-bounded-derivatives|Sobolev embedding into bounded continuous derivatives]]
- [[functional-analysis/weighted-cutoff-sobolev-estimate|Sobolev estimate with a scaled cutoff]]
- [[functional-analysis/three-dimensional-sobolev-inequality|Three-dimensional Sobolev inequality]]

### Linear algebra

- [[linear-algebra/kronecker-delta|Kronecker delta]]

### Partial differential equations

- [[partial-differential-equations/harmonic-sobolev-distribution-vanishes|A harmonic global Sobolev distribution vanishes]]
- [[partial-differential-equations/harmonic-distribution|Harmonic distribution]]
- [[partial-differential-equations/newtonian-fundamental-solution-three-dimensions|Newtonian fundamental solution in three dimensions]]
- [[partial-differential-equations/newtonian-potential|Newtonian potential in three dimensions]]
- [[partial-differential-equations/poisson-equation|Poisson equation]]

### Real and multivariable analysis

- [[real-analysis/mollifier|Mollifier]]

## B12 · Energy, uniqueness, breakdown and periodization

22 new knowls.

### Convex geometry

- [[convex-analysis/subquadratic-absorption|Absorbing a subquadratic power]]

### Fluid dynamics

- [[fluid-dynamics/change-of-viscosity-by-spatial-rescaling|Changing viscosity by spatial rescaling]]
- [[fluid-dynamics/classical-energy-uniqueness|Classical uniqueness from difference energy]]
- [[fluid-dynamics/cutoff-pressure-flux-estimate|Cutoff pressure-flux estimate for a velocity difference]]
- [[fluid-dynamics/delayed-parabolic-rescaling|Delayed parabolic rescaling at a fixed terminal time]]
- [[fluid-dynamics/navier-stokes-difference-equation|Difference equation for two incompressible flows]]
- [[fluid-dynamics/energy-bound-from-integrable-forcing|Energy bound from a time-integrable L2 force]]
- [[fluid-dynamics/kinetic-energy-identity|Global kinetic-energy identity]]
- [[fluid-dynamics/pressure-gradient-identification|Identifying pressure gradients without a pressure growth assumption]]
- [[fluid-dynamics/solenoidal-integration-cancellation|Integration cancellation for divergence-free transport and pressure]]
- [[fluid-dynamics/kinetic-energy-inequality|Kinetic-energy inequality]]
- [[fluid-dynamics/local-kinetic-energy-balance|Local kinetic-energy balance for smooth incompressible flow]]
- [[fluid-dynamics/localized-difference-energy-identity|Localized difference-energy identity]]
- [[fluid-dynamics/periodization-of-separated-navier-stokes-fields|Periodization of separated incompressible flows]]
- [[fluid-dynamics/relative-kinetic-energy|Relative kinetic energy]]
- [[fluid-dynamics/bounded-kinetic-energy|Uniformly bounded kinetic energy]]
- [[fluid-dynamics/uniqueness-against-compact-reference-flow|Uniqueness against a smooth compactly supported reference flow]]
- [[fluid-dynamics/viscous-dissipation|Viscous dissipation in the incompressible energy balance]]

### Fourier and harmonic analysis

- [[harmonic-analysis/periodization-of-compactly-supported-function|Periodization of a compactly supported smooth function]]

### Linear algebra

- [[linear-algebra/frobenius-norm|Frobenius norm]]

### Partial differential equations

- [[partial-differential-equations/sobolev-continuation-obstruction|Supremum growth obstructs continuous Sobolev continuation]]

### Real and multivariable analysis

- [[real-analysis/unbounded-path-prevents-continuous-extension|Unbounded values approaching a point prevent continuous extension]]

## B13 · Nonlinear residual correction and asymptotic summation

17 new knowls.

### Asymptotic estimates

- [[asymptotics/shrinking-cutoff-asymptotic-summation|Asymptotic summation by shrinking cutoffs]]
- [[asymptotics/diagonal-choice-of-shrinking-scales|Diagonal choice of shrinking scales]]
- [[asymptotics/log-square-exponential-decay|Exponential decay in the square of a logarithm]]
- [[asymptotics/infinite-order-decay-with-derivatives|Infinite-order decay with derivatives]]

### Foundations

- [[shared-foundations/recursion-on-natural-numbers|Recursion on the natural numbers]]

### Fourier and harmonic analysis

- [[harmonic-analysis/rapid-decay-of-shifted-torus-integral|Rapid decay of a shifted torus integral]]

### Function spaces and operators

- [[functional-analysis/finite-derivative-bookkeeping-on-a-dag|Derivative requirements on a finite computation graph]]
- [[functional-analysis/frozen-linear-inverse-residual-update|Residual update with a fixed linear inverse]]

### Partial differential equations

- [[partial-differential-equations/differential-polynomial-difference-estimate|Difference estimate for a differential polynomial]]
- [[partial-differential-equations/flat-residual-from-asymptotic-summation|Flat residual from asymptotic summation]]
- [[partial-differential-equations/localized-characteristic-primitive|Localized characteristic primitive with an explicit defect]]
- [[partial-differential-equations/shifted-characteristic-primitive|Shifted characteristic primitive on a torus]]

### Real and multivariable analysis

- [[real-analysis/solenoidal-asymptotic-summation|Asymptotic summation preserving divergence freedom]]
- [[real-analysis/cutoff-of-a-controlled-scale|Derivatives of a cutoff composed with a controlled scale]]
- [[real-analysis/one-sided-jet-limits-from-derivative-bounds|One-sided jet limits from uniform derivative bounds]]
- [[real-analysis/smooth-extension-from-bounded-endpoint-jets|Smooth extension from bounded endpoint jets]]
- [[real-analysis/weighted-shifted-radial-primitive|Weighted shifted radial primitive]]

## B14 · Convex integration and weak-solution context

29 new knowls.

### Differential geometry

- [[differential-geometry/h-principle|The h-principle]]

### Fluid dynamics

- [[fluid-dynamics/active-scalar-equation|Active scalar equation]]
- [[fluid-dynamics/averaged-navier-stokes-model|Averaged Navier–Stokes model]]
- [[fluid-dynamics/caffarelli-kohn-nirenberg-theorem|Caffarelli–Kohn–Nirenberg partial regularity theorem]]
- [[fluid-dynamics/endpoint-l3-regularity-criterion|Endpoint L-infinity-in-time L3 regularity criterion]]
- [[fluid-dynamics/euler-subsolution|Euler subsolution with prescribed energy]]
- [[fluid-dynamics/hypodissipative-navier-stokes|Hypodissipative Navier–Stokes equations]]
- [[fluid-dynamics/incompressible-porous-media-equation|Incompressible porous media equation]]
- [[fluid-dynamics/leray-projection|Leray projection on Euclidean space]]
- [[fluid-dynamics/leray-hopf-solution|Leray–Hopf solution]]
- [[fluid-dynamics/local-energy-inequality|Local energy inequality]]
- [[fluid-dynamics/navier-stokes-energy-class|Navier–Stokes energy class]]
- [[fluid-dynamics/reynolds-averaging-stress|Reynolds averaging stress]]
- [[fluid-dynamics/reynolds-defect-system|Reynolds defect system]]
- [[fluid-dynamics/singular-set-of-weak-solution|Singular set of a weak velocity]]
- [[fluid-dynamics/suitable-weak-solution|Suitable weak solution]]
- [[fluid-dynamics/weak-navier-stokes-solution|Weak Navier–Stokes solution]]

### Fourier and harmonic analysis

- [[harmonic-analysis/fractional-laplacian|Fractional Laplacian on Euclidean space]]
- [[harmonic-analysis/order-zero-fourier-multiplier|Order-zero Fourier multiplier]]

### Function spaces and operators

- [[functional-analysis/weakly-continuous-path|Weakly continuous path]]

### Partial differential equations

- [[partial-differential-equations/backward-uniqueness|Backward uniqueness]]
- [[partial-differential-equations/convex-integration|Convex integration]]
- [[partial-differential-equations/differential-inclusion|Differential inclusion]]
- [[partial-differential-equations/first-order-differential-relation|First-order differential relation]]
- [[partial-differential-equations/formal-and-holonomic-solution|Formal and holonomic solutions]]
- [[partial-differential-equations/nonuniqueness|Nonuniqueness for an initial-value problem]]
- [[partial-differential-equations/partial-regularity|Partial regularity]]
- [[partial-differential-equations/regularity-criterion|Regularity criterion]]
- [[partial-differential-equations/weak-formulation|Weak formulation of a differential equation]]

## Additional document integration

### Real and multivariable analysis

- [[real-analysis/geometric-series|Geometric series]] — added while connecting the complete paper to the batch collection.
