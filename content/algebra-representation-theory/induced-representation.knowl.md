...
The **induced representation** is the $G$-representation
$$
\operatorname{Ind}_H^G W:=\mathbb{C}[G]\otimes_{\mathbb{C}[H]}W,
$$
where $G$ acts by left multiplication on the first factor. It has dimension
$$
...
Equivalently, $\operatorname{Ind}_H^G W$ can be realized on
$$
\mathrm{Ind}_H^G W
=\Bigl\{f:G\to W\mid f(gh)=\sigma(h)^{-1}f(g)
\text{ for all }g\in G,\ h\in H\Bigr\},
$$
with action
$$
