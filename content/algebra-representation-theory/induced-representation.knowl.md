+++
id = "algebra-representation-theory/induced-representation"
...
Using the [[algebra-representation-theory/group-algebra|group algebra]], induction can be realized as
\[
\mathrm{Ind}_H^G W \;\cong\; \mathbb{C}[G]\otimes_{\mathbb{C}[H]} W,
\]
where \(\mathbb{C}[G]\) is viewed as a \((\mathbb{C}[G],\mathbb{C}[H])\)-[[algebra-modules/bimodule|bimodule]] and \(\otimes\) is the [[algebra-modules/tensor-product|tensor product]] over \(\mathbb{C}[H]\).

## Relationship with restriction (Frobenius reciprocity)

Let \(\mathrm{Res}_H^G V\) denote the [[algebra-representation-theory/restricted-representation|restricted representation]] of a \(G\)-representation \(V\) to \(H\). Then induction is left adjoint to restriction: there is a natural vector space isomorphism
\[
\mathrm{Hom}_G(\mathrm{Ind}_H^G W,\; V)\;\cong\;\mathrm{Hom}_H(W,\; \mathrm{Res}_H^G V),
\]
where \(\mathrm{Hom}\) denotes [[algebra-modules/module-homomorphism|module homomorphisms]] in the appropriate categories.

## Examples

### Example 1: Inducing the trivial representation gives a permutation representation
Let \(\sigma\) be the trivial \(1\)-dimensional representation of \(H\). Then \(\mathrm{Ind}_H^G \sigma\) is naturally isomorphic to the permutation representation of \(G\) on the set of left [[algebra-groups/coset|cosets]] \(G/H\).

Special case: if \(H=\{e\}\), then \(G/H\cong G\) and \(\mathrm{Ind}_{\{e\}}^G \mathbf{1}\) is the [[algebra-representation-theory/regular-representation|regular representation]].
...
\]
i.e. a \(1\)-dimensional invariant subspace (spanned by \((1,1,1)\)) plus the \(2\)-dimensional standard representation (compare [[algebra-representation-theory/completely-reducible-representation|complete reducibility]]).
