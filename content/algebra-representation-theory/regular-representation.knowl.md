+++
id = "algebra-representation-theory/regular-representation"
...
The **left regular representation** of \(G\) over \(k\) is the [[algebra-representation-theory/group-representation|representation]]
\[
\lambda: G \longrightarrow \mathrm{GL}\big(k[G]\big)
\]
on the [[algebra-representation-theory/group-algebra|group algebra]] \(k[G]\), where \(k[G]\) is the \(k\)-vector space with basis \(\{e_g : g\in G\}\) and the action is
\[
...
Over \(\mathbb C\), the [[algebra-representation-theory/character|character]] \(\chi_{\mathrm{reg}}\) of the regular representation satisfies
\[
...
If \(\{V_i\}\) runs over the [[algebra-representation-theory/irreducible-representation|irreducible representations]] of \(G\) over \(\mathbb C\), with \(\dim V_i = d_i\), then the regular representation decomposes as
\[
...
\]
which is the content of [[algebra-representation-theory/sum-squares-degrees|the sum of squares formula]]. The index \(i\) ranges over exactly as many irreducibles as there are [[algebra-groups/conjugacy-class|conjugacy classes]] (see [[algebra-representation-theory/number-irreducibles-conjugacy-classes|number of irreducibles equals number of conjugacy classes]]).
