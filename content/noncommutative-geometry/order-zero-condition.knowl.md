...
(a\xi)b=a(\xi b)
\]
holds precisely because \(a\) commutes with \(b^\circ\). Thus the order-zero axiom makes \(H\) an \(\mathcal A\)-bimodule in the represented sense. The involution in \(b^\circ=Jb^*J^{-1}\) is needed for this right action to be complex-linear and multiplicative.
