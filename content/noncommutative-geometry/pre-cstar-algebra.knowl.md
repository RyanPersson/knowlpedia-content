...
The involution is isometric and therefore extends continuously to the Banach-space completion \(\overline A\). Multiplication extends as well, and the \(C^*\)-identity passes to limits, so \(\overline A\) is a \(C^*\)-algebra in which \(A\) is a dense \(*\)-subalgebra. Conversely, every \(*\)-subalgebra of a \(C^*\)-algebra, equipped with the inherited norm, is a pre-\(C^*\)-algebra. These standard completion facts follow from the basic \(C^*\)-norm identities in [Murphy, §2.1].
