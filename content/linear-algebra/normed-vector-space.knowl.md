...
A **normed vector space** is a [[linear-algebra/vector-space|vector space]]
over \(\mathbb{R}\) or \(\mathbb{C}\), equipped with a
[[linear-algebra/norm|norm]] \(\|\cdot\|\) on \(V\).
...
For example, on \(\mathbb{R}^2\) the Euclidean, \(\ell^1\), and max norms give
different lengths but the same notion of convergence. In infinite dimensions,
different norms on the same vector space need not define the same topology.
