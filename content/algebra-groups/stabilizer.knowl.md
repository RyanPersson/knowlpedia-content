...
The stabilizer \(G_x\) is always a [[algebra-groups/subgroup|subgroup]] of \(G\).
...
Together with the [[algebra-groups/orbit|orbit]] \(G\cdot x\), it controls the action: the [[algebra-groups/orbit-stabilizer-theorem|orbit-stabilizer theorem]] gives a bijection between \(G/G_x\) and \(G\cdot x\), and in the finite case implies \(|G\cdot x| = [G:G_x]\).
