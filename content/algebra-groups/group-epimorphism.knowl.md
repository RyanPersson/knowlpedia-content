...
Equivalently, \(\varphi\) is an epimorphism if and only if its [[algebra-groups/image-group|image]] equals all of \(H\), i.e. \(\varphi(G)=H\).

## Remarks

Many natural epimorphisms arise as quotient maps: if \(N\lhd G\) then the canonical projection \(G\to\) [[algebra-groups/quotient-group|quotient group]] \(G/N\) is surjective.
