...
If \(G\) is a [[lie-groups/complex-lie-group|complex Lie group]], its **underlying real Lie group**, denoted \(G_{\mathbb R}\), has the same points, multiplication, and inversion as \(G\), but its complex manifold is regarded as a smooth real manifold. If \(\dim_{\mathbb C}G=n\), then
\[
\dim_{\mathbb R}G_{\mathbb R}=2n.
\]
This construction is functorial on holomorphic group homomorphisms.

## What is and is not forgotten

Only the scalar field of the manifold charts is forgotten. The topology and abstract group are unchanged. The complex structure remains recoverable as an additional left-invariant endomorphism \(J\) of the real tangent bundle satisfying \(J^2=-1\), but \(J\) is not part of \(G_{\mathbb R}\).
