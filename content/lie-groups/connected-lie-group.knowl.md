+++
id = "lie-groups/connected-lie-group"
title = "Connected Lie group"
kind = "knowl"
summary = "A Lie group whose underlying smooth manifold is connected (equivalently, equal to its identity component)."
aliases = ["connected-lie-group", "Connected Lie group"]
domains = ["lie-groups"]
legacy_source_path = "lie-groups/connected-lie-group.md"
prerequisites = ["fiber-bundles/lie-group", "topology/connected-set"]
dependency_heuristic = "semantic-spotcheck-review-v1"
dependency_review_count = 2
+++

A **connected Lie group** is a [[fiber-bundles/lie-group|Lie group]] \(G\) whose underlying [[topology/topological-space|topological space]] (hence smooth manifold) is connected.

A basic structural fact is that every Lie group has a distinguished connected, normal Lie subgroup: the **identity component**
\[
G^\circ := \text{the connected component of } e \in G.
\]
Then:

- \(G^\circ\) is an open-and-closed embedded [[lie-groups/lie-subgroup|Lie subgroup]] and a [[lie-groups/normal-lie-subgroup|normal Lie subgroup]] of \(G\).
- The connected components of \(G\) are precisely the left (or right) cosets of \(G^\circ\).
- The quotient \(G/G^\circ\) is a discrete group (often called the component group).

## Remarks

The Lie algebra [[lie-groups/lie-algebra-of-a-lie-group|\(\mathfrak g = \mathrm{Lie}(G)\)]] controls the local connected structure through the [[lie-groups/exponential-map-lie-group|exponential map]]: for any [[topology/neighborhood|neighborhood]] \(U\subset \mathfrak g\) of \(0\), the subgroup generated by \(\exp(U)\) equals \(G^\circ\). In particular, \(G\) is connected iff it is generated by exponentials of elements of \(\mathfrak g\).

**Context.** Many classification and representation-theoretic statements are stated for connected (or connected compact) groups because the discrete component group \(G/G^\circ\) contributes separate, essentially “finite/discrete” data on top of the Lie-algebraic information.
