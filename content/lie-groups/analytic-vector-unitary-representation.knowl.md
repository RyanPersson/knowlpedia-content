...
Nelson proved that analytic vectors are dense for continuous representations of finite-dimensional Lie groups, using heat-kernel regularization [Nelson, pp. 589–595](https://doi.org/10.2307/1970331). For translations of \(\mathbb R\) on \(L^2(\mathbb R)\), vectors whose Fourier transforms have a square-integrable exponential weight are analytic. A vector with only polynomial Fourier decay can be smooth while failing the factorial convergence test.
