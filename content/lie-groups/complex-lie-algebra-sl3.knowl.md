...
For the Cartan subalgebra of diagonal trace-zero matrices, write \(\varepsilon_i\) for the \(i\)-th diagonal coordinate. The roots are
\[
\varepsilon_i-\varepsilon_j\qquad(i\ne j),
\]
and one may take \(\varepsilon_1-\varepsilon_2\) and \(\varepsilon_2-\varepsilon_3\) as simple roots. The corresponding Dynkin diagram has two nodes joined by one edge.
