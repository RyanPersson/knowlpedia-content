...
The **complex Lie algebra** \(\mathfrak{so}_{10}(\mathbb C)\) consists of endomorphisms preserving a nondegenerate symmetric bilinear form infinitesimally:
\[
...
Its distinguished irreducible modules include the vector module \(\mathbf{10}\), the two inequivalent half-spin modules \(\mathbf{16}_+\) and \(\mathbf{16}_-\), and the adjoint module \(\mathbf{45}\cong\Lambda^2\mathbf{10}\).

## Spin representations and global groups

The half-spin modules are representations of the [[lie-groups/spin-group|spin group]] \(\operatorname{Spin}(10,\mathbb C)\), the simply connected complex group with Lie algebra \(\mathfrak{so}_{10}(\mathbb C)\). They do not descend to the ordinary matrix group \(SO(10,\mathbb C)\). Thus the notation “\(SO(10)\) grand unified theory” commonly refers globally to \(\operatorname{Spin}(10)\) when a \(\mathbf{16}\) of fermions is present.

The compact real form integrates to compact \(\operatorname{Spin}(10)\); the split real form is \(\mathfrak{so}(5,5)\). Other real forms, such as \(\mathfrak{so}(p,10-p)\), have the same complexification but different real-group representation theory.
...
\]
is followed by \(\mathfrak{so}_{10}\subset [[lie-groups/exceptional-lie-algebra-e6|\mathfrak e_6]]\). Under \(\mathfrak{so}_{10}\oplus\mathbb C\), the adjoint representation of \(\mathfrak e_6\) branches, up to a choice of charge sign, as
\[
