...
G/Z,
\]
whose multiplication is \((gZ)(hZ)=ghZ\). Centrality makes \(Z\) normal, and closedness gives the coset space its canonical smooth Lie-group structure.
...
\]
When \(Z\) is discrete—for example, finite—the quotient map \(G\to G/Z\) is a local diffeomorphism and the two groups have isomorphic Lie algebras. A finite central quotient can therefore change global topology and available representations without changing the infinitesimal symmetry algebra.
