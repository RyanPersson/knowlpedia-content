+++
id = "stat-mech-lattice/xy-model"
title = "XY model"
kind = "knowl"
summary = "O(2)-symmetric lattice spin model with planar (angle) spins on the circle, featuring continuous symmetry and (in 2D) a Kosterlitz–Thouless transition."
aliases = ["xy-model", "XY model"]
domains = ["stat-mech-lattice"]
legacy_source_path = "stat-mech-lattice/xy-model.md"
+++

The **XY model** (also called the planar rotor or O(2) model) is a lattice spin system with **continuous** spins taking values on the unit circle. It is a canonical example of a model with continuous symmetry in the setting of [[stat-mech-lattice/configuration-space-lattice|lattice configuration spaces]].
...
A standard nearest-neighbor [[stat-mech-lattice/lattice-hamiltonian|Hamiltonian]] with an [[stat-mech-lattice/external-field-coupling|external field]] $\mathbf{h}\in\mathbb{R}^2$ is
$$
H_\Lambda(\theta)
={}
...
\qquad J\in\mathbb{R}.
$$
More generally one can include boundary terms via a [[stat-mech-lattice/boundary-condition-lattice|boundary condition]].
...
Its hallmark is the interplay between **continuous symmetry**, **spin-wave fluctuations**, and (in 2D) **topological defects** (vortices), producing phase behavior not captured by discrete-spin models like the [[stat-mech-lattice/ising-model|Ising model]].
