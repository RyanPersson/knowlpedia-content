+++
id = "stat-mech-lattice/potts-model"
title = "Potts model"
kind = "knowl"
summary = "q-state lattice spin model with permutation symmetry, generalizing the Ising model and closely related to the random-cluster model."
aliases = ["potts-model", "Potts model"]
domains = ["stat-mech-lattice"]
legacy_source_path = "stat-mech-lattice/potts-model.md"
+++

The **Potts model** is a lattice spin system in which each site carries one of $q$ discrete states (“colors”). It generalizes the [[stat-mech-lattice/ising-model|Ising model]] from two spin values to $q\ge 2$ values.

Let $\Lambda$ be a finite subset of $\mathbb{Z}^d$ (or a finite graph) and fix an integer $q\ge 2$. A [[stat-mech-lattice/spin-configuration|spin configuration]] is a map $\sigma:\Lambda\to\{1,2,\dots,q\}$, i.e. the [[stat-mech-lattice/spin-space|spin space]] is $\{1,\dots,q\}$. With boundary condition $\eta$ on $\Lambda^c$, a standard nearest-neighbor [[stat-mech-lattice/lattice-hamiltonian|Hamiltonian]] is
$$
...
where $J\in\mathbb{R}$ controls the interaction and $(h_1,\dots,h_q)$ is an [[stat-mech-lattice/external-field-coupling|external field]] favoring certain states.
...
- **Symmetry.** With $h_a\equiv 0$, the model is invariant under permutations of the $q$ states (the symmetry group is $S_q$). Ordered phases correspond to selecting a preferred state, captured by an [[stat-mech-lattice/order-parameter|order parameter]] such as the deviation of state densities from $1/q$.

- **Reduction to Ising at $q=2$.** For $q=2$, the Potts model is equivalent (after a simple reparameterization and additive energy shift) to the [[stat-mech-lattice/ising-model|Ising model]].

- **Random-cluster (Fortuin–Kasteleyn) representation.** In the ferromagnetic case, the Potts model is tightly linked to the [[stat-mech-lattice/random-cluster-model|random-cluster model]]: the Potts partition function can be rewritten as a sum over bond configurations with a cluster weight $q$. This connection is a key tool for studying [[stat-mech-lattice/phase-transition-gibbs|phase transitions]] and correlations.
