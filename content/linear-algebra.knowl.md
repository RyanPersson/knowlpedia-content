+++
id = "linear-algebra"
title = "Linear Algebra"
kind = "section"
summary = "Vector spaces, linear maps, inner products, and spectral theory"
aliases = ["linear-algebra", "Linear Algebra"]
domains = ["linear-algebra"]
legacy_source_path = "linear-algebra/_index.md"
+++

Click any term to expand its definition inline.

---

## Vector Spaces

- [[linear-algebra/vector-space|Vector space]]
- [[linear-algebra/euclidean-space|Euclidean space]]

---

## Linear Maps

- [[linear-algebra/linear-map|Linear map]]
- [[linear-algebra/linear-operator|Linear operator]]

---

## Eigentheory

- [[linear-algebra/eigenvalue|Eigenvalue]]
- [[linear-algebra/eigenvector|Eigenvector]]
- [[linear-algebra/eigenspace|Eigenspace]]
- [[linear-algebra/characteristic-polynomial|Characteristic polynomial]]
- [[linear-algebra/minimal-polynomial|Minimal polynomial]]

---

## Matrix Invariants

- [[linear-algebra/determinant|Determinant]]
- [[linear-algebra/positive-semidefinite-matrix|Positive semidefinite matrix]]
- [[linear-algebra/trace|Trace]]

---

## Inner Products and Norms

- [[linear-algebra/inner-product|Inner product]]
- [[linear-algebra/inner-product-space|Inner product space]]
- [[linear-algebra/bilinear-form|Bilinear form]]
- [[linear-algebra/orthogonality|Orthogonality]]
- [[linear-algebra/norm|Norm]]
- [[linear-algebra/normed-vector-space|Normed vector space]]
- [[linear-algebra/euclidean-norm|Euclidean norm]]
- [[linear-algebra/operator-norm|Operator norm]]

---

## Function Spaces

- [[linear-algebra/banach-space|Banach space]]
- [[linear-algebra/hilbert-space|Hilbert space]]
- [[linear-algebra/compact-operator|Compact operator]]

---

## Theorems

- [[linear-algebra/basis-existence-theorem|Existence of a basis]]
- [[linear-algebra/rank-nullity-theorem|Rank-nullity theorem]]
- [[linear-algebra/cayley-hamilton-theorem|Cayley–Hamilton theorem]]
- [[linear-algebra/cauchy-schwarz-inequality|Cauchy–Schwarz inequality]]

---

## Conjecture dependencies

- [[linear-algebra/unit-sphere|Unit sphere of a normed space]]
- [[linear-algebra/surjective-linear-isometry|Surjective linear isometry]]
- [[linear-algebra/orthonormal-basis|Orthonormal basis]]
- [[linear-algebra/closed-linear-subspace|Closed linear subspace]]
- [[linear-algebra/orthogonal-complement|Orthogonal complement]]
- [[linear-algebra/orthogonal-projection|Orthogonal projection]]
- [[linear-algebra/operator-invariant-subspace|Invariant subspace of an operator]]

## Quaternionic and Hermitian linear algebra

- [[linear-algebra/complexification|Complexification]]
- [[linear-algebra/quaternion-division-algebra|Quaternion division algebra]]
- [[linear-algebra/quaternionic-vector-space|Quaternionic vector space]]

## Quadratic, Hermitian, and projective linear algebra

- [[linear-algebra/realification-of-a-complex-vector-space|Realification of a complex vector space]]
- [[linear-algebra/quadratic-form|Quadratic form]]
- [[linear-algebra/signature-of-symmetric-bilinear-form|Signature of a symmetric bilinear form]]
- [[linear-algebra/hermitian-matrix|Hermitian matrix]]
- [[linear-algebra/minkowski-vector-space|Minkowski vector space]]
- [[linear-algebra/semilinear-map|Semilinear map]]
