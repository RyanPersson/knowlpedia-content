...
For example, the map \(\mathbb R\to S^1\), \(t\mapsto e^{it}\), is a local diffeomorphism even though it is not globally one-to-one. The inverse function theorem says that a smooth map between manifolds of equal dimension is a local diffeomorphism wherever its derivative is invertible.
