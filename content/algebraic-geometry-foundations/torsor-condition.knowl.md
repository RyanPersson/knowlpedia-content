...
For \(P=\operatorname{Spec}K\to X=\operatorname{Spec}F\), two points of \(P\) lying over the same point of \(X\) should differ by one and only one Galois automorphism. The torsor condition packages that sentence without choosing geometric points.
...
**Warning.** Checking that an action is free and transitive merely on ordinary topological points is generally too weak. Scheme structure, residue fields, and nilpotents are detected by the displayed isomorphism but may be invisible on the underlying point set.
