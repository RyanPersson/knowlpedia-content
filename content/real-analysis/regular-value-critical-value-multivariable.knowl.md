...
A value \(b\) is a **critical value** if it is not a [[fiber-bundles/regular-value|regular value]]; equivalently, there exists \(a\in U\) such that \(f(a)=b\) and \(\operatorname{rank}Df(a)<m\).

## Remarks

Regular values are those at which the level set \(f^{-1}(b)\) is expected to be a smooth \((n-m)\)-dimensional set (under appropriate hypotheses). [[real-analysis/critical-value|Critical values]] correspond to "singular" level sets.

## Examples

- For \(f:\mathbb{R}^2\to\mathbb{R}\), \(f(x,y)=x^2+y^2\), the value \(0\) is a critical value (attained at the [[real-analysis/critical-point|critical point]] \((0,0)\)), while every \(b>0\) is a regular value.
