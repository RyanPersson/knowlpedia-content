+++
id = "differential-geometry/stokes-theorem"
...
\]
When \(M\) is compact, the compact-support condition is automatic.
...
## Useful corollaries
- If \(\partial M=\varnothing\), then for any compactly supported \(\alpha \in \Omega^{n-1}(M)\),
  \[
  \int_M d\alpha = 0.
  \]
