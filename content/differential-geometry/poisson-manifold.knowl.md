...
\]
has image tangent to a canonical, generally singular foliation. Each leaf carries a symplectic form, and its dimension is the even [[linear-algebra/rank|rank]] of \(\pi\).
...
**Warning.** Sign conventions differ. Here \(X_f(g)=\{f,g\}\) and \(\beta(\pi^\sharp\alpha)=\pi(\alpha,\beta)\). Authors who define Hamiltonian vector fields or the inverse of a symplectic form with the opposite sign obtain the opposite displayed Poisson bracket. “Poisson space” may also denote a singular or algebraic object; this knowl defines only the smooth-manifold notion.
