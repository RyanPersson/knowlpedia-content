...
For \(m\in\mu^{-1}(\alpha)\), the kernel of \(i^*\omega\) equals the tangent space to the \(G_\alpha\)-orbit. Thus the restricted form is basic for the free proper action and descends to a nondegenerate two-form. Closedness descends from \(d\omega=0\) [Ortega and Ratiu, §4.3](https://doi.org/10.1007/978-1-4757-3811-7).
...
For an abelian group every coadjoint value is fixed, so nonzero levels are reduced by the full group. For a nonabelian group, \(G_\alpha\) can be strictly smaller, and the dimension formula reflects that difference.
...
**Warning.** If regularity, freeness, or properness fails, the orbit space still exists set-theoretically but need not be a manifold; that setting belongs to [[differential-geometry/singular-symplectic-reduction|singular symplectic reduction]].
