...
Let \((M,\omega)\) be a \(2n\)-dimensional [[differential-geometry/symplectic-manifold|symplectic manifold]], and let \(F=(F_1,\ldots,F_n):M\to\mathbb R^n\) have pairwise commuting components whose differentials are independent along \(F^{-1}(c)\). If \(\Lambda\) is a compact connected component of this [[differential-geometry/regular-level-set|regular fiber]], the **Liouville–Arnold theorem** states that \(\Lambda\) is diffeomorphic to \(\mathbb T^n\) and has a saturated neighborhood carrying [[differential-geometry/action-angle-coordinates|action-angle coordinates]]. In those coordinates the fibers of \(F\) are the tori \(I=\mathrm{constant}\), and every Hamiltonian commuting with all \(F_j\) depends only on the actions. Consequently, its flow is linear on each such torus [Arnol'd, Chapter 10](https://doi.org/10.1007/978-1-4757-1693-1).
