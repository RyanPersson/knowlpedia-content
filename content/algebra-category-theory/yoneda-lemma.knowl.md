+++
id = "algebra-category-theory/yoneda-lemma"
title = "Yoneda lemma"
kind = "knowl"
...
Let \(\mathcal C\) be a [[algebra-category-theory/category|category]] such that each hom-class \(\mathrm{Hom}_{\mathcal C}(A,B)\) is a set (i.e. \(\mathcal C\) is *locally small*). Fix an [[algebra-category-theory/object|object]] \(A\in\mathcal C\) and a [[algebra-category-theory/functor|functor]] \(F:\mathcal C\to \mathbf{Set}\).
...
- Given \(\eta\in \mathrm{Nat}(h^A,F)\), the corresponding element of \(F(A)\) is
  ...
  \]
  where \(\mathrm{id}_A\) is the [[algebra-category-theory/identity-morphism|identity morphism]] of \(A\).
...
- Given \(x\in F(A)\), define a natural transformation \(\eta^x:h^A\Rightarrow F\) by, for each object \(X\),
  ...
  f \mapsto F(f)(x).
  \]
  Naturality follows from functoriality of \(F\) and [[algebra-category-theory/composition-category|composition]] in \(\mathcal C\).

This bijection is in fact a [[algebra-category-theory/natural-isomorphism|natural isomorphism]] between functors in \(A\) and \(F\).
