+++
id = "fiber-bundles/differential-pushforward-of-a-smooth-map"
...
\]
between [[fiber-bundles/tangent-space-at-a-point|tangent spaces]] defined as follows (using the derivation model of tangent vectors): if $v\in T_pM$ is a derivation at $p$, then $dF_p(v)\in T_{F(p)}N$ is the derivation given by
\[
(dF_p(v))(g) := v(g\circ F)
\]
...
dF: TM \to TN,
\]
covering $F$ (meaning $\pi_N\circ dF = F\circ \pi_M$, where $\pi_M:TM\to M$ and $\pi_N:TN\to N$ are the bundle projections). This viewpoint uses the [[fiber-bundles/tangent-bundle|tangent bundle]] functorially.
