+++
id = "fiber-bundles/splitting-of-the-atiyah-sequence"
...
prerequisites = ["fiber-bundles/atiyah-sequence-tpgtm0", "fiber-bundles/vector-bundle-morphism"]
...
A **splitting** of the Atiyah sequence is a [[fiber-bundles/vector-bundle-morphism|vector bundle map]]
\[
\sigma:TM\to TP/G
\]
...
Splittings are in natural bijection with [[fiber-bundles/principal-connection|principal connections]] on \(P\):
...
The curvature of the induced connection can be recovered from \(\sigma\) as the obstruction to \(\sigma\) preserving brackets, using the negative-fundamental-vector inclusion \(\operatorname{ad}(P)\hookrightarrow TP/G\), in the Atiyah algebroid: the \(\mathrm{ad}(P)\)-valued 2-form
\[
...
## Examples
1. **Trivial bundle and a gauge potential.** For \(P=M\times G\), use the identification \(TP/G\cong TM\oplus (M\times \mathfrak{g})\) in which the second coordinate is the negative vertical component, as in the linked Atiyah algebroid. A choice of \(\mathfrak{g}\)-valued 1-form \(A\) gives a splitting \(\sigma(v)=(v,A(v))\), whose associated principal connection has local connection form \(A\).
