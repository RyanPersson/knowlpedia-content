+++
id = "fiber-bundles/transition-function"
...
Let \(\pi:E\to M\) be a smooth fiber bundle with typical fiber \(F\), and let \((U_i,\Phi_i)\) and \((U_j,\Phi_j)\) be [[fiber-bundles/local-trivialization|local trivializations]]. On the overlap \(U_{ij}=U_i\cap U_j\), the change of trivialization is the map
\[
\Phi_{ij}:=\Phi_i\circ \Phi_j^{-1}:U_{ij}\times F\longrightarrow U_{ij}\times F.
\]
It is a [[fiber-bundles/diffeomorphism|diffeomorphism]] over \(\mathrm{id}_{U_{ij}}\), so it necessarily has the form
\[
...
3. **Vector bundles:** in a [[fiber-bundles/vector-bundle|vector bundle]] of rank \(k\), transition functions take values in \(\mathrm{GL}(k,\mathbb{R})\) because trivializations are fiberwise linear.
