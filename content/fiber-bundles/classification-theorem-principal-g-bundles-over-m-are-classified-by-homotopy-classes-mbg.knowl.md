...
**Classification theorem.** The assignment
\[
...
2. **Circle.** Principal \(G\)-bundles over \(S^1\) are classified by conjugacy classes in \(\pi_0(G)\). In particular, they are all trivial when \(G\) is connected. For \(G=O(1)\cong\{\pm1\}\), there are two classes; the nontrivial class gives the Möbius line bundle through the associated-bundle construction.
