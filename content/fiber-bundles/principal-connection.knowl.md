+++
id = "fiber-bundles/principal-connection"
...
\]
A principal connection can also be encoded by a [[fiber-bundles/connection-1-form-on-a-principal-bundle|connection 1-form]] \(\omega\) on \(P\); its kernel at each point is the horizontal subspace.

A principal connection determines horizontal lifts of curves and hence a notion of [[fiber-bundles/parallel-transport|parallel transport]] in \(P\).
...
2. **Connections on frame bundles.** If \(M\) has a linear connection on its [[fiber-bundles/tangent-bundle|tangent bundle]], then the frame bundle \(\mathrm{Fr}(TM)\to M\) carries an induced principal connection whose horizontals correspond to parallel frames along curves.
