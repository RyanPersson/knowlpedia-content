+++
id = "fiber-bundles/transition-matrix-of-a-local-frame"
...
\]
that is [[fiber-bundles/smooth-map|smooth]] and satisfies, for each $x\in U\cap V$ and each $j=1,\dots,r$,
\[
...
2. **Möbius [[fiber-bundles/line-bundle|line bundle]].** The Möbius real line bundle over $S^1$ can be described by two trivializations whose overlap transition function is the constant map $g(\theta)=-1\in \mathrm{GL}(1,\mathbb R)$. This single nontrivial transition function encodes the twist.

3. **Orientation check.** For a real bundle, on $U\cap V$ the sign of $\det g(x)$ detects whether the two frames determine the same [[fiber-bundles/orientation-of-a-real-vector-bundle|orientation]] (positive determinant) or the opposite one (negative determinant).
