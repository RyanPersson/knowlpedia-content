+++
id = "fiber-bundles/curvature-2-form-of-a-principal-connection"
...
\]
where \(d\) is the [[fiber-bundles/exterior-derivative|exterior derivative]] and \([\omega\wedge\omega]\) is formed using the [[fiber-bundles/lie-bracket|Lie bracket]] on \(\mathfrak{g}\) together with the wedge product of 1-forms.

Key properties:
- \(\Omega\) is **horizontal**: \(\Omega(X,\cdot)=0\) whenever \(X\) is vertical.
- \(\Omega\) is **equivariant**: \(R_g^*\Omega=\mathrm{Ad}(g^{-1})\Omega\).

Consequently, \(\Omega\) represents the [[fiber-bundles/curvature|curvature]] of the underlying [[fiber-bundles/principal-connection|principal connection]] and can be viewed as an \(\mathrm{ad}(P)\)-valued 2-form on \(M\) after descending from \(P\).
