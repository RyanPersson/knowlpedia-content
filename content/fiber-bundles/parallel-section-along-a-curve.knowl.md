...
Let \(\pi:E\to M\) be a smooth vector bundle with a [[fiber-bundles/connection-on-a-vector-bundle|connection]] \(\nabla\), and let \(\gamma:I\to M\) be a smooth curve on an interval \(I\). A **section along \(\gamma\)** is a smooth section \(s\) of the pullback bundle \(\gamma^*E\), equivalently a smooth map \(s:I\to E\) with \(\pi\circ s=\gamma\). It is **parallel along \(\gamma\)** if its covariant derivative for the pullback connection vanishes:
\[
\frac{D s}{dt}(t)=0 \quad \text{for all } t\in I.
\]

## Equivalent characterizations
Given \(t_0\in I\) and \(v_0\in E_{\gamma(t_0)}\), there is a unique parallel section \(s\) along \(\gamma\) with \(s(t_0)=v_0\). Evaluating this section at another time gives the [[fiber-bundles/parallel-transport|parallel transport]] determined by \(\nabla\) along \(\gamma\).
