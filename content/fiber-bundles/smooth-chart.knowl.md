...
Let $M$ be an $n$-dimensional [[fiber-bundles/smooth-manifold|smooth manifold]]. A **(topological) chart** on $M$ is a pair $(U,\varphi)$ where $U\subset M$ is open and $\varphi:U\to V$ is a homeomorphism onto an open set $V\subset \mathbb{R}^n$.
...
\]
is smooth (as a map between open subsets of $\mathbb{R}^n$). Equivalently, $\varphi$ is a [[fiber-bundles/diffeomorphism|diffeomorphism]] between the open submanifold $U$ (with the induced smooth structure) and the open set $V\subset \mathbb{R}^n$.
