...
Let \(M\) be an \(n\)-dimensional [[fiber-bundles/smooth-manifold|smooth manifold]], let \(FM\to M\) be its frame bundle, and let \(\theta\) be the [[fiber-bundles/solder-form-on-the-frame-bundle|solder form]]. Given a [[fiber-bundles/principal-connection|principal connection]] on \(FM\), with connection \(1\)-form \(\omega\in\Omega^1(FM;\mathfrak{gl}(n,\mathbb R))\), its **torsion \(2\)-form** is
\[
