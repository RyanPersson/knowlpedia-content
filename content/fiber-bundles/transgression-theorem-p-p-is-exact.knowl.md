...
Let \(\pi:Q\to M\) be a [[fiber-bundles/principal-g-bundle|principal \(G\)-bundle]], let \(\omega_0,\omega_1\) be [[fiber-bundles/principal-connection|principal connections]] with curvatures \(\Omega_0,\Omega_1\), and let \(p\) be an \(\operatorname{Ad}\)-invariant real- or complex-valued homogeneous polynomial of degree \(k\ge1\) on \(\mathfrak g\).
...
where
\[
\pi^*\mathrm{CS}_p(\omega_0,\omega_1)
=k\int_0^1p(\eta,\Omega_t,\dots,\Omega_t)\,dt.
\]
Here \(\mathrm{CS}_p\) is the [[fiber-bundles/transgression-form|transgression form on the base]]; the displayed basic form on \(Q\) specifies its pullback uniquely. The symmetric polarization of \(p\) is evaluated on one \(1\)-form and \(k-1\) curvature \(2\)-forms. Consequently, the de Rham class of \(\operatorname{cw}_p(\omega)\) is independent of \(\omega\).
...
2. **Degree 2 (classical 3D Chern–Simons).** For a matrix group and \(P(X)=\mathrm{tr}(X^2)\), the transgression from the product flat reference connection (local potential zero) to the connection with local potential \(A\) gives the usual \(3\)-form on a trivialization:
   \[
