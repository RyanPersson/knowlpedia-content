...
1. **Trivial bundle on one chart.** On \(U\) with a trivialization, a local gauge function \(u:U\to G\) sends a \(\mathfrak g\)-valued 1-form \(A\) to
   ...
   \]
   For an abelian group, such as \(U(1)\), \(\mathrm{Ad}\) is trivial and this reduces to \(A^u=A+u^{-1}du\).

2. **A pure gauge is gauge-equivalent to zero.** Starting with \(A=0\), gauge transformation by \(u\) gives \(A^u=u^{-1}du\), the [[fiber-bundles/pure-gauge-connection-ag-1dg-on-a-trivial-bundle|pure gauge connection]]. Applying the inverse change of gauge recovers
   \[
   (A^u)^{u^{-1}}=0,
   \]
   the [[fiber-bundles/flat-connection-a0-on-a-trivial-bundle|flat connection]] in the original gauge.

3. **Changing local sections changes the clutching data by cocycle equivalence.** If a principal bundle is presented by a transition function \(g\) on a two-set cover, replacing the local sections by \(s_1'=s_1u_1\) and \(s_2'=s_2u_2\) changes it to
   \[
   g'=u_1^{-1}gu_2.
   \]
   This does not change the bundle's isomorphism class. When \(u_1=u_2\) on the overlap, the formula specializes to conjugation.
