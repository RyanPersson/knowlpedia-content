+++
id = "fiber-bundles/construction-product-principal-bundle-when-defined"
...
\]
Because \(\pi_P\) and \(\pi_Q\) are submersions, \(P\times_M Q\) is an [[differential-geometry/embedded-submanifold|embedded submanifold]] of \(P\times Q\), and the map
\[
...
The canonical projections \(P\times_M Q\to P\) and \(P\times_M Q\to Q\) are [[fiber-bundles/principal-bundle-morphism|principal bundle morphisms]] covering \(\mathrm{id}_M\).

## Examples
1. If \(P=M\times G\) and \(Q=M\times H\) are trivial, then \(P\times_M Q \cong M\times (G\times H)\) as principal \((G\times H)\)-bundles.
2. If \(P\) is the oriented orthonormal frame bundle of a [[differential-geometry/riemannian-manifold|Riemannian manifold]] and \(Q\) is a principal circle bundle over the same base, then \(P\times_M Q\) is a principal \((\mathrm{SO}(n)\times S^1)\)-bundle encoding both structures simultaneously.
3. If \(H=G\) and \(Q=P\), then \(P\times_M P\) is a principal \((G\times G)\)-bundle; its quotient by the diagonal action of \(G\) is canonically identified with the [[fiber-bundles/construction-adjoint-bundle-ad|adjoint bundle]] (as a bundle of groups) when \(G\) acts by conjugation.
