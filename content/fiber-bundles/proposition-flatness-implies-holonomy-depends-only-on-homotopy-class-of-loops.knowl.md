+++
id = "fiber-bundles/proposition-flatness-implies-holonomy-depends-only-on-homotopy-class-of-loops"
...
prerequisites = ["fiber-bundles/principal-g-bundle", "fiber-bundles/principal-connection", "fiber-bundles/holonomy-group", "fiber-bundles/curvature", "topology/homotopy", "topology/fundamental-group", "algebra-groups/group-homomorphism"]
dependency_heuristic = "semantic-full-review-v1"
dependency_review_count = 1
+++

Let \(\pi:P\to M\) be a [[fiber-bundles/principal-g-bundle|principal G-bundle]] with a [[fiber-bundles/principal-connection|principal connection]] \(\omega\). Fix a basepoint \(x\in M\) and a point \(p\in P_x\). For a piecewise smooth loop \(\gamma\) based at \(x\), horizontal lifting gives an element \(\mathrm{Hol}_{p}(\gamma)\in G\), and the subgroup generated by such elements is the [[fiber-bundles/holonomy-group|holonomy group]] at \(p\). Let \(\Omega\) denote the [[fiber-bundles/curvature|curvature]] of \(\omega\).
...
\]
Using the loop-product convention that the right factor is traversed first, the assignment \([\gamma]\mapsto \mathrm{Hol}_{p}(\gamma)\) defines a group homomorphism
\[
...
2. **Flat \(U(1)\)-connections on \(S^1\).** For a connection with constant local 1-form \(A=i\lambda\,d\theta\) (in a global gauge), curvature is zero and holonomy sends the generator of \(\pi_1(S^1)\cong \mathbb Z\) to \(e^{-2\pi i\lambda}\in U(1)\); dependence is only on the winding number.
