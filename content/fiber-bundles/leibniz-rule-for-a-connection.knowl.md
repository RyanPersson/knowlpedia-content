...
prerequisites = ["fiber-bundles/vector-bundle", "fiber-bundles/vector-field", "fiber-bundles/module-of-smooth-sections"]
dependency_heuristic = "semantic-spotcheck-review-v1"
dependency_review_count = 2
+++

Let \(\mathbb K=\mathbb R\) or \(\mathbb C\), let \(E\to M\) be a smooth \(\mathbb K\)-vector bundle, and consider an operation \((X,s)\mapsto\nabla_Xs\) on smooth vector fields and smooth sections, with values in smooth sections. Assume it is additive in each variable, \(\mathbb K\)-linear in the section variable, and \(C^\infty(M;\mathbb R)\)-linear in the real vector field \(X\). For \(f\in C^\infty(M;\mathbb K)\), the identity below is the additional axiom that defines a [[fiber-bundles/connection-on-a-vector-bundle|connection]].
...
2. **Constant scalars.** If \(f\) is constant, then \(X(f)=0\) and the rule becomes \(\nabla_X(fs)=f\,\nabla_X s\), expressing homogeneity over \(\mathbb K\)-constants in the section argument. Additivity is part of the separately assumed bilinearity.
