+++
id = "fiber-bundles/equivalent-bundle-atlases"
...
On overlaps between a chart from \(\mathcal A\) and a chart from \(\mathcal B\), the compatibility is expressed by smooth [[fiber-bundles/transition-function|transition functions]] taking values in \(\mathrm{Diff}(F)\).
