...
\]
In particular, \(dF_e\) maps \(\ker(d\pi_e)\) into \(\ker(d\pi'_{F(e)})\), so it sends the [[fiber-bundles/vertical-tangent-space|vertical tangent space]] at \(e\) into the vertical tangent space at \(F(e)\). On the level of the [[fiber-bundles/tangent-bundle|tangent bundles]], this says \(dF:TE\to TE'\) is compatible with the vertical subbundles.

## Examples
1. **Differential of a smooth map:** for a [[fiber-bundles/smooth-map|smooth map]] \(\varphi:M\to N\), the differential \(d\varphi:TM\to TN\) is a bundle map over \(\varphi\).
