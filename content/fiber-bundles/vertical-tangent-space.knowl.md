+++
id = "fiber-bundles/vertical-tangent-space"
...
T_e(E_x)=V_eE.
\]
In particular, \(\dim(V_eE)=\dim(E)-\dim(M)\), so the vertical tangent space has constant dimension along each connected component of \(E\). As \(e\) varies, the spaces \(V_eE\) assemble into the [[fiber-bundles/vertical-subbundle|vertical subbundle]] \(VE\subset TE\).
...
3. **Principal bundle:** if \(\pi:P\to M\) is a [[fiber-bundles/principal-g-bundle|principal G-bundle]] with structure group \(G\) and [[lie-groups/lie-algebra|Lie algebra]] \(\mathfrak g\), then \(V_pP\) is the tangent space to the orbit \(p\cdot G\), and the fundamental vector field construction gives a linear isomorphism \(\mathfrak g\cong V_pP\).
