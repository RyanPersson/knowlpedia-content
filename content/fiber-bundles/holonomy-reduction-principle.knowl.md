...
prerequisites = ["fiber-bundles/smooth-manifold", "fiber-bundles/principal-g-bundle", "fiber-bundles/lie-group", "lie-groups/lie-subgroup", "fiber-bundles/principal-connection", "fiber-bundles/holonomy-group", "fiber-bundles/principal-h-subbundle"]
...
\]
Then there exists a [[fiber-bundles/principal-h-subbundle|principal \(H\)-subbundle]] \(Q\subseteq P\) containing \(p\) (an \(H\)-reduction of structure group) with the following properties:
...
## Parallel-section characterization

Equivalently: there exists such an \(H\)-reduction preserved by \(\omega\) if and only if the associated bundle \(P\times_G (G/H)\to M\) admits a [[fiber-bundles/section-of-a-fiber-bundle|global section]] that is parallel with respect to the induced connection (i.e., constant under [[fiber-bundles/parallel-transport|parallel transport]]).
...
3. **Holonomy contained in the identity gives a global parallel frame.**
   If \(\mathrm{Hol}_p(\omega)=\{e\}\) on connected \(M\), then parallel transport is path-independent and produces a global trivialization by parallel frames; in particular one gets a reduction to the trivial subgroup.
