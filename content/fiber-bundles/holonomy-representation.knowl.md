...
Let \(\pi:P\to M\) be a principal \(G\)-bundle with a [[fiber-bundles/flat-principal-connection|flat principal connection]]. Fix a basepoint \(x\in M\) and \(p\in P_x\).

For a loop \(\gamma\) based at \(x\), let \(g_\gamma\in G\) be the element determined by horizontal lifting as in the definition of the [[fiber-bundles/holonomy-group|holonomy group]]:
\[
