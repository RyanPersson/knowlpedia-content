...
If \(\operatorname{char}(F)\nmid n\), then \(\zeta\) is a primitive \(n\)th root of unity if and only if it is a root of the [[algebra-fields-galois/cyclotomic-polynomial|cyclotomic polynomial]] \(\Phi_n(x)\). This characterization can fail after reduction to characteristic dividing \(n\).
