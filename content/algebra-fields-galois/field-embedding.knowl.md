...
prerequisites = ["algebra-rings/field", "algebra-rings/ring-homomorphism"]
...
2. **Two \(\mathbb{Q}\)-embeddings of a quadratic field.** Let \(L=\mathbb{Q}(\sqrt{d})\) with \(d\ne1\) squarefree. Then there are two \(\mathbb{Q}\)-embeddings \(L\hookrightarrow \mathbb{C}\): one sends \(\sqrt{d}\mapsto \sqrt{d}\), the other sends \(\sqrt{d}\mapsto -\sqrt{d}\).
